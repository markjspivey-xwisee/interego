...
- **The overall bounds come from one pass over the axes**, not from spreading them into `Math.min`/`Math.max`. An attestation can carry any number of axes, and spreading throws a `RangeError` past the engine's argument limit, about 125,000 in Node (the automated review of #494).
- The result is still deterministic for the same inputs.

`tests/registry.test.ts` pins `now` at moments the old arithmetic got wrong, and asserts that first so the tests cannot pass on the clock's luck: 0.8 at 2026-09-26T08:15:09Z, 0.9 at 08:00:06Z, and two agreeing 0.9s at 08:00:00Z. It also covers a weightless ancient attestation, three equal axes, and a mixed mean equal to the plain one. Against the previous build, the exactness tests fail. An attestation with 300,000 axes aggregates without throwing; with the spread it fails with a `RangeError`. Four mutants were checked, and each fails the tests: axes unheld, the overall score unheld, weightless scores widening the range, and a wrong bound.
...
- **One check for both ways in** (`bridge/server.ts`). `foreignPodRefusal(args, wallet)` holds the rule: `membershipHolds` with `POD_OWNERSHIP`, and the typed `wrongPod` refusal (403, `iep:refusalStatus`) naming the caller's own pod. The configured tenant, whose directory is administered rather than self-enrolled, is still exempt. The envelope path calls it with the signer, as before. The token path now calls it with the token's verified wallet (`verified.token.address`), once the token verifies and before its context is built.
- **Tests** (`tests/enrollment-ownership.test.ts`). Real session tokens from a private seed, for a squatter and a pod owner, both with rows in the owner's pod's membership. The squatter's token verifies against that membership, and its wallet does not hold the pod, so it is refused. The owner's token verifies and its wallet holds the pod, so it gets through. A source check pins the wiring in `resolveCaller`: both calls, the token one after verification and before the context. Four mutants were checked, and each fails the tests: the token call removed, the envelope call removed, the ownership test dropped from the helper, and the token call moved after the context is built. The mutation gate's `wrongPod-drops-its-status` anchor is unchanged and still caught.
...
- **Shared externals.** The new test is the second suite that reaches `lti13.ts`, which reads `FOXXI_LTI_PRIVATE_KEY_PEM`. So `applications/_shared/tests/shared-live-externals.ts` registers it in `SHARED_ONLY_THROUGH_IMPORTED_CODE`: it is a key that decides which keypair signs, not an address.
...
## 2026-09-25 — Foxxi: a republished course catalog changes on the pod, and an owner's survives a restart

`foxxi.publish_course_catalog_product` publishes under one fixed slug so that a republish replaces the catalog, but `publish()` writes a graph create-only and tolerates the 412. Every republish kept the first graph beside a new descriptor and reported success, so a catalog never changed after its first publish and its descriptor described a graph the pod did not serve. It now clears the old copy first, as the tenant's own sections do (`src/tenant-publisher.ts`). An owner's catalog listed the SCORM courses they authored from the bridge's in-process cache alone, which every restart empties, so the first publish after a deploy offered nothing. It now also reads the author's own lattice and pod records, as course hydration does, and lists each course once (`authoredCourseProducts` in `src/course-catalog-product.ts`, with tests). Both were found running the live demo against the deployed bridge.

## 2026-09-25 — Foxxi: a failed attempt earns nothing; credentials through any signed connection; a pod's owner publishes its catalog

A fix and two openings, found building a demo that runs through a person's own relay connection. **The fix**: the SCORM engine records `completed` beside `failed` for an attempt that did not pass, and `completed` counted as mastery, so a failed attempt could be claimed; mastery is now `passed`, `mastered`, cmi5 `satisfied` or `waived`, and completing a course is not passing it. **Signed claims**: `foxxi.claim_credential_signed` and `foxxi.earned_credentials_signed` (`POST /agent/credentials/claim` and `/earned`) claim a credential for a course the bridge's SCORM engine grades, signed the way the engine's own routes are — DIRECT by a wallet, or DELEGATED by an agent whose delegation credential the signer anchors, which is how a relay connection (a Claude connector) reaches Foxxi. They read the signer's own pod, derived from the signature and never named by the caller; a delegated claim is issued to the person the delegation names; the credential's criteria name the course's author. The standings follow the record: every course it is about, plus any asked for. `foxxi.claim_credential` and `earned_credentials` could not serve a relay connection, whose signatures all recover to the relay's one key. **Owner publishing**: `foxxi.publish_course_catalog_product` with `tenant_pod_url` naming a self-sovereign pod is its owner's to publish, to that pod and issued by it (it read the named pod's catalog but published to, and credited, the configured tenant), with the SCORM courses the owner authored on this bridge, each port the course's own IRI. The claim hint `earned_credentials` returns now carries the `tenant_pod_url` the claim needs.

## 2026-09-24 — Foxxi: courses as federated data products

HyprCat has said since it was written how a catalog federates — a `FederatedCatalog` of `FederatedDataProduct`s, each issued by an identity and placed in one of three worlds, with distributions that are affordances — and nothing in any vertical instantiated it; Foxxi's course catalog was one JSON blob in a pod section, typed only inside its payload, so no manifest said "this pod publishes a catalog" and no peer could find a course without holding the pod and the section's name. `foxxi.publish_course_catalog_product` (admin) renders the tenant's catalog as HyprCat Turtle — the catalog in the service world, issued by the tenant, each course a `FederatedDataProduct` with its title, category, audience keywords, standard, landing page and an output port that is a followable distribution (a GET of the course's own IRI), peer catalogs named by `hyprcat:federatedWith` — and publishes it to the tenant pod as a public Asserted descriptor that conforms to `hyprcat:FederatedCatalog`, under one IRI per tenant pod so a republish supersedes. `foxxi.discover_course_catalogs` walks the pods the caller names, or the tenant pod and this deployment's federation peers, finds catalogs by that descriptor type in each manifest, reads them back, checks the catalog's issuer against the identity the manifest attributes the descriptor to, and reports every pod walked, an unreachable one included. The vertical uses the Layer 2 pattern as written and adds no term to it. Pure render, parse and discovery in `src/course-catalog-product.ts`, with tests.
## 2026-09-24 — Foxxi: credentials earned from evidence, on the learner's own pod

The tenant could issue an Open Badges 3.0 credential into a learner's pod wallet, but only from claims an admin typed in; nothing looked at what the learner had done, nothing told a learner what they were ready to claim, and a verifier that checked the signature checked nothing else. `foxxi.claim_credential` issues a completion credential for a catalog course only from the learner's own record: a passed, completed, mastered, satisfied or waived xAPI statement about the course (any of the course's IRIs, or a statement whose parent or grouping is one), with success not false and any score at or above the course's threshold, and graded by the bridge itself (the SCORM engine's completions carry a grading tag keyed by a secret only the bridge holds, so a statement the learner recorded is in the record but does not earn a credential); the credential names that evidence, is signed by the tenant's issuer key, is valid for a year, is written to the learner's wallet, and a `credentialed` statement joins their record. Held and in force, the credential comes back unissued; not earned, a 409 refusal says what statement would earn it. `foxxi.earned_credentials` is where every assigned course stands — credentialed, claimable (with the claim call), in progress, not started, a lapsed credential named — read from the learner's shared lattice, lens and durable records and their wallet. `foxxi.verify_credential` is what a relying party should check, open to anyone: the proof verifies and its key is the stated issuer, the validity window holds, the issuer is one this tenant stands behind, the credential names its subject; each failed check in words. A learner acts for themselves, an admin for any learner; the pod read is bounded to this deployment's pod space. Pure logic in `src/earned-credentials.ts`, with tests; `achievementIdFor` in `src/credentials.ts` names the achievement before issuing so a held credential is matched first.

## 2026-09-24 — every affordance as an agent skill

Each vertical declares its capabilities once in `affordances.ts`, and the bridges derive the manifest, the MCP tool schemas, the SHACL input shapes and the OpenAPI document from it; the one form agent runtimes actually load, an agentskills.io `SKILL.md`, was written by hand for a single vertical. `tools/build-skills.ts` now derives one skill per vertical from the same declarations, with `@interego/skills` emitting the frontmatter: `docs/skills/<vertical>/SKILL.md` says what the vertical is for, the three ways to invoke any of its affordances (an Interego MCP connector's `invoke_affordance` against the bridge's manifest, the bridge's own `POST /mcp`, or HTTP), and a table of them; `reference.md` beside it carries every description and input; `docs/skills/README.md` indexes the eleven (Foxxi's learner and administration surfaces apart, LLM telemetry as its own since it is served by the Foxxi bridge). Each affordance lands in the skill whose vertical its action IRI names, so nothing appears twice. `tests/skills-from-affordances.test.ts` fails when a committed file differs from a fresh build or a stale file sits beside them, and checks every `SKILL.md` parses under the package's agentskills.io rules; `--check` does the same from the shell. The harness README now links the hand-written skill where it lives in the tree, since `.claude/` is ignored there.


## 2026-09-23 — the Foxxi bridge reads the pod a few entities at a time

Every content-judgment call that needs the pod's judgments, outcomes, attestations or autonomy policies read them one entity at a time. On the fifth live run of the loop on one pod (over a hundred judgments and as many outcomes) `foxxi.cross_confirm` took longer than the runner's 120-second budget, the runner gave up on a call the bridge then finished on its own, and the steps after it never ran. `src/bounded.ts` (`readEach`) reads with at most eight in flight, results in the listing's order, an unreadable entity left out as before; the five pod walks in the bridge use it. With tests: order, failures left out, never more than the limit in flight.

## 2026-09-23 — the judgment loop, domain-free: the kit is the harness for any decision

`applications/_shared/judgment-kit/loop.ts` states what jev-harness built for code and Foxxi built again for content, with no nouns from either. A decision kind is a name, its alternatives and the registry axis its hit rate reports on. `judgment()` normalises a model's or an agent's distribution over the alternatives; `outcome()` scores a confirmation (hit, multiclass Brier, who confirmed and whether as a person or an agent, the peer judgment when a judge confirmed); `calibration()` holds a cell per kind, Asserted from the floor, people and agents counted apart; `attestation()` is the registry attestation a calibration earns (each Asserted kind's hit rate on its axis, kinds sharing an axis pooled by sample, honesty from the Brier, Self when the judge attests itself and Peer otherwise); `reputations()` and `rankJudges()` say who has earned what on which kind; `crossConfirmations()` and `awaitingAPerson()` are what judges owe each other and what a person still owes; `autonomy()` is the three checks the harness's auto-merge and Foxxi's autonomy policy make, in the order they fail. `applications/_shared/tests/judgment-loop.test.ts` runs the whole loop in memory on a decision kind neither vertical has, an incident's severity. The kit's README says what each vertical takes from it and what stays in the vertical: the question, the storage, the publishing and the authorization. Built through the harness (navigation mudmm9h5-4ed070).

## 2026-09-23 — content that grades and revises itself

`foxxi.weakest_claims` ranks every evidence-level claim judged on the pod, weakest first: graded by a person's confirmation if one exists, else an agent's, else the judges' answers (the newest per judge, the lowest of them when they disagree, because a claim two judges grade apart is not yet known to be well supported), each with the judgments and confirmations behind the grade and a revise control. `foxxi.revise_claim` records a `foxxi:ContentRevision` — the claim as it stood and the grade it had, the words now and the evidence cited from outside the passage, who revised it and whether as a person or an agent — and has the bridge's judge grade the revised claim immediately under the autonomy policy, so the record says the grade before and after; a revision must change the words or add evidence, and the original judgment stands. Pure logic in `src/revisions.ts`, with tests; `tools/content-judgment-loop.revisions.json` revises the two demo-course claims the judge graded lowest, and the runner's `--revise` walks it.

## 2026-09-23 — autonomy granted by calibration, as a constitutional policy

A `foxxi:AutonomyPolicy` entity on the pod says, per kind of question, the registry rating on its axis a judge must reach (accuracy for evidence-level, competence for work-regime), over how many outcomes the latest attestation about it must rest, and how many of those a person must have confirmed; the default is the harness's bar for merging without a person (0.9 over 20, 5 by a person). `foxxi.judge_content_claim` and `foxxi.record_content_judgment` now publish Asserted without a person only when the judge clears that rule, Hypothetical otherwise, and cite the decision and its reason either way. `foxxi.set_autonomy_policy` amends the policy through `@interego/constitutional` — proposed, voted by the caller, ratified under `DEFAULT_RULES` for the tier: a self-sovereign pod is tier 4 (one vote, in force at once), the configured tenant tier 3 (a quorum of three, published Hypothetical until it ratifies) — each policy superseding the last. `foxxi.autonomy_status` is the table of judge × kind decisions with the standing and the rule behind each. Pure logic in `src/autonomy.ts`, with tests; the runner prints each judgment's decision and the table.

## 2026-09-23 — a network of judges: every judgment names its judge, judges confirm each other, and `best_judge` says who has earned what on which question

A `foxxi:ContentJudgment` carries `judge` (the bridge's own model, or the agent that recorded it); `foxxi.record_content_judgment` lets any enrolled agent record a judgment it made itself, with its answer, its distribution and the model it declares, as a judge in its own right. `foxxi.cross_confirm` scores every two judges' newest judgments of one claim by each other's answer, once each way, as agent confirmations that name the peer judgment (`confirmedFrom`); an outcome carries the judge it scores, so calibrations are per judge. `foxxi.attest_content_judgments` publishes one attestation per judge — Self about the bridge's own, Peer about the others — superseding that judge's earlier one; `foxxi.content_judgment_reputation` lists every judge's registry snapshot; `foxxi.best_judge` ranks them for a kind of question by the axis that kind is scored on (accuracy for evidence-level, competence for work-regime). `foxxi.confirm_next` now retires a judgment only on a person's confirmation, and puts the claims judges disagree on first (weights 0.4 uncertainty, 0.2 cell need, 0.1 evidence weakness, 0.3 disagreement). The runner's `--record-own <model>` records the claims file's answers as the caller's own judgments before the bridge's model judges, then cross-confirms, then asks who is best. Pure logic in `src/judges.ts`, with tests.

## 2026-09-23 — the two content questions, sharpened where the live run showed the rubric let two readings pass

Eleven live confirmations read strictly against the written criteria say the model was right in three of the four evidence-level misses: a passage that corrects a claim makes it `unsupported`, a slide that merely states one makes it `asserted-only`. The fourth miss was the rubric's: an evidence item that only names the claim's own slide counted as cited evidence, so the two middle levels could not be told apart whenever a caller cited the slide. `supported-by-cited-evidence` and `corroborated` now require evidence from outside the passage, and `supported-by-context` says that citing the passage is context. On work-regime the model answered Evident for every claim phrased as a practice, and the old wording allowed it ("an established practice applies"); Evident now requires that following the practice determines the result, and Emergent names the case where the result turns on how other people or the situation respond. `tools/content-judgment-loop.claims.json` confirms by the criteria as written (five answers changed, with the reason in each note). Re-run on the same pod after deploy: 8 of 11 hits against 4 of 11 (work-regime 5 of 5 against 2 of 5; both Emergent claims now judged Emergent at 1.0 and 0.9), cumulative calibration evidence-level hit 0.417, work-regime hit 0.7 brier 0.443, a second attestation and a registry snapshot from two. The three evidence-level misses that remain are the confirmer's, not the model's: by the sharpened wording the slide item is context, so one outside item is supported-by-cited-evidence, not corroborated, and a slide that states an instruction is asserted-only. The claims file now says so.

## 2026-09-23 — the content-judgment loop is self-sovereign, and an outcome says who confirmed it

The six loop affordances (`judge_content_claim`, `confirm_content_judgment`, `content_judgment_calibration`, `attest_content_judgments`, `content_judgment_reputation`, `confirm_next`) take `tenant_pod_url`: omitted, the configured tenant and its learning-engineer or admin roles as before; naming another pod, that self-sovereign pod, whose owner (a member whose WebID lives under it, which is what self-enrolment produces) runs the whole loop there. The identity that runs this repository's live proofs is a valid signer and no member of the configured tenant, and the bridge's own refusal names self-enrolment as the way out; this is what lets it close the loop as itself. `confirm_content_judgment` takes `confirmed_by_kind` (human by default, or agent), the outcome records it, calibration cells carry `humanSamples` and `agentSamples`, and the attestation carries `confirmers`, so an agent's reading — a second model's opinion — is never dressed as a person's. `tools/content-judgment-loop.ts` drives the loop end to end, signed with the wallet it reads, from a claims file; `tools/content-judgment-loop.claims.json` holds eleven claims from the demo course — six evidence-level and five work-regime, so one run reaches both sample floors. Run live on 2026-09-23 as the Claude Code agent on its own pod: eleven judgments, eleven agent-confirmed outcomes (four hits), both cells Asserted, the first attestation (accuracy 0.333, competence 0.4, honesty 0.546; confirmers human 0, agent 11) and a registry snapshot from it. What the run showed about the questions: the work-regime Choice answered Evident for every claim, twice at confidence 1.0, including two a careful reading calls Emergent; the evidence-level Score disagreed with that reading four times in six.

## 2026-09-22 — the suite runs as four parts: the wall is now the adversarial module plus a runner's minute

Two parts measured 5:05 for the four slowest modules on one runner against 3:50 for everything else, so the wall was the slow part. The suite now runs as four: `tests/workspace-adversarial.test.ts` alone (155 s, the floor), the next five slowest together (workspace-desktop-renderer, solid, cas-split, bounded-manifest, egress-dns-screen), and everything else in two `--shard`s. `tests/suite-partition.test.ts` holds every cut: the named and excluded modules are the same set, every complement part excludes that same set, and the shards are 1/n … n/n once each; the live-externals registry's test checks that no complement part excludes a pod suite. Under `--shard` vitest hands the run-integrity reporter every surviving specification and runs only the shard's slice, so the first four-part run called the other half "never finished"; `tools/vitest-run-integrity.mjs` now plans vitest's own slice (`shardPlan`: ceil(total / n) per shard, the last one shorter), and a shard whose worker dies still fails it.
...
## 2026-09-22 — the gated auto-merge waits for the whole suite: it had been red on every pull request since the morning, and seven merged over it

bridge-typecheck.yml runs the whole root suite (422 modules, 6,600 tests) on every pull request, ten minutes, and since it left master pushes this morning nothing waited for it: the judge job merged on its selected tests while the suite ran on, into a check nobody read. From 02:24Z to 16:31Z it was red on every pull request — `tests/workflow-trigger-covers-the-suite.test.ts` still asserted the two `paths:` lists the workflow no longer has, and no changed file imported that test, so the selection never picked it — and #446 to #452 merged automatically over it. The two assertions now say what the workflow is (one list, pull requests only); the judge job runs `tools/wait-for-workflow-run.mjs` before its decision, polling the runs listed for the head until the suite has concluded (a listing without the job's own run is short, not green), and `applications/jev-harness/src/auto-merge.ts` takes the word as its last condition: success or absent holds, anything else refuses. The merge now lands when the suite does, about ten minutes after the push instead of two; `JEV_MERGE_WAITS_FOR_SUITE=false` makes the word advisory for an operator who wants the two back.

## 2026-09-22 — the live client-signature check runs outside the rollout lock, and the deploy gate stops waiting for it

The post-deploy synthetic approval test was a `uses:` of `client-signature-live.yml` inside `auto-deploy.yml`, so it ran in the run that holds the fleet's rollout lock (concurrency group `auto-deploy-master`): 18 minutes on run 35753761634, during which the next merge's deploy sat queued for 21 of its 27 minutes from merge to live. The deploy run now dispatches the check as its own run (`gh workflow run`, `actions: write`) and ends when the deploys are recorded; `tools/ci-green-for-sha.mjs` drops that workflow's runs from its listing (`POST_DEPLOY_WORKFLOWS`), because a dispatched run lands on whatever master is by then and would otherwise hold that commit's deploy for the same 18 minutes. The test pins both: the ignored name is the workflow's `name:`, and `auto-deploy.yml` dispatches rather than `uses:` it.
...
## 2026-09-21 — the judgment kit: what jev-harness proved, shared; Foxxi judges content with it

`applications/_shared/judgment-kit` holds what any vertical needs to publish a System One judgment as an Interego artifact: the System One client and its fake (moved from jev-harness, re-exported there), the Turtle literal and IRI helpers, the payload prefixes the relay's note projection recognises, the control triples, the seven-facet descriptor in TriG, the HyperMarkdown projection, and the scoring arithmetic (hit@1, hit@3, Brier). jev-harness now renders through it, byte for byte as before (its own tests pinned the extraction). Foxxi gains `foxxi.judge_content_claim`: one narrow question to the model about a unit of course content — evidence-level, a Score over a five-level evidence scale, or work-regime, a Choice among Foxxi's own regimes — answered as typed probabilities with the model's confidence, published to the tenant pod as a Hypothetical `foxxi:ContentJudgment` with the controls a person is offered (cite-evidence, choose-method-for-regime, confirm-or-refute). Learning-engineer or admin only, because the model is metered to the operator; 503 without `TYPESAFE_API_KEY`.

## 2026-09-21 — jev-harness: navigation consults the pod's memory of what earlier tasks changed

A navigation outcome now carries the task and the observed files (`jvh:task`, `jvh:observedFile`), the pod read-back keeps them, and `HarnessStore.precedents()` lists them across local and pod outcomes. `navigate()` consults precedents whose task resembles the new one (lexical similarity in code, never the model), injects their files into the candidate set when the directory pass dropped them, and mixes their distribution into the model's with a weight proportional to the strongest similarity; the judgment records what was consulted and what contributed (`jvh:precedent`, `jvh:precedentWeight`, `jvh:precedentsConsulted`). `tools/backtest.ts` replays oldest first and takes `--precedents none|run|store` so the effect is measured without leakage. Measured on 40 commits: memory changed no hit (hit@1 0.28 → 0.33 came from two commits without precedents), lowered mean Brier 0.158 → 0.126 and mean confidence 0.503 → 0.447; outcomes record the memory share (`jvh:priorPrecedentWeight`) and the calibration view splits navigation with memory from without, so live tasks decide.
...
- A cmi5 submission retry MUST retain its original result and statement IDs, and MUST NOT resend outcomes that the LRS already acknowledged.
...
- Sealed publications defer plaintext SHACL checks explicitly to clients and report
  the caller and container shape requirements without claiming validation. Their
  recipient count comes from the supplied envelope; the relay does not infer
  recipient identities or author inclusion from its own managed audience.
