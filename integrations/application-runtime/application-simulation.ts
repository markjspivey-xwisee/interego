/** L3, pure preview of the existing signed-domain/v1 interpreter. */
import {
...
export interface SimulationInput {
  ...
  /** Limit evaluation to this declared action. */
  readonly actionIri?: string;
  /** False evaluates only the supplied samples, without enumerating a frontier. */
...
export interface SimulationFrontier {
  ...
  readonly coverage: readonly { readonly actionIri: string; readonly inputSpace: 'finite' | 'open'; readonly enumerated: boolean; readonly suppliedSamples: number }[];
...
export function simulateApplication(resolved: ResolvedApplicationLab, input: SimulationInput): SimulationFrontier {
  ...
  if (input.actionIri && !actionIds.has(input.actionIri)) throw new Error('action is absent from active contract');
  ...
  for (const action of actions) {
    if (input.actionIri && action.actionIri !== input.actionIri) continue;
    const enumerated = input.enumerate !== false;
    const { payloads, open } = enumerated ? enumerate(action, budget) : {
      payloads: [], open: (action.inputs ?? []).filter(i => !i.options && i.type !== 'boolean').map(i => i.name),
    };
    const samples = (input.samples ?? []).filter(s => s.actionIri === action.actionIri);
    coverage.push({ actionIri: action.actionIri, inputSpace: open.length ? 'open' : 'finite', enumerated, suppliedSamples: samples.length });
    if (enumerated && open.length) {
