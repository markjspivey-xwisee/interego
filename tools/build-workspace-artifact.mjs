...
export async function block() {
  ...
  return [
    ...
    '/* ★ NO `entryTurtle` ADAPTER IS BOUND, AND THE ONE THAT USED TO BE HERE WAS A LIE.',
    '   It took four positional arguments — streamIri, seq, body, prior — and the comment above',
    '   it said "every call site below passes them that way". No call site below passed them at',
    '   all: the page appends through `postEntry`, which composes the entry itself. Worse, the',
    '   adapter omitted `author`, which `entryTurtle` refuses — so the one thing it could have',
    '   done was throw. Dead code whose comment asserts a fact about the file is worse than dead',
    '   code, because the next reader believes it. */',
    '',
    ...
    '/* ★ AND SEPARATELY, WHAT ONE ENTRY WAS SPOKEN ON. Being somebody\'s delegate is standing;',
    '   whether a particular sentence was said FOR them is per-entry, and `footingLine` is the',
    '   one sentence every surface shows for it. Bound, not restated: the desktop shell and the',
    '   Discord conduit render the same value, and a page with its own wording for it would be',
    '   the place the distinction quietly softened. */',
    'const readEntryAuthorship = WSPC.readEntryAuthorship, authorshipLine = WSPC.authorshipLine;',
    'const footingLine = WSPC.footingLine;',
