    ...
    <div style={{ maxWidth: 1020, margin: '0 auto', padding: '28px 24px 80px' }}>
      ...
      {result && (
        <div style={{ ...card, marginTop: 16, borderLeft: `5px solid ${result.landed?.readBack ? 'var(--good)' : result.validVerifiable?.conforms ? 'var(--good)' : 'var(--warn)'}` }}>
          ...
          <div style={{ display: 'grid', gap: 3, fontSize: 12 }}>
            ...
            {result.verifiable.proofOk !== undefined && <div>◆ <strong>confidence committed range proof (≥ 0.7, Pedersen gap-hiding)</strong> <span style={{ color: result.verifiable.proofOk ? 'var(--good)' : 'var(--bad)' }}>{result.verifiable.proofOk ? 'verified' : 'unverified'}</span></div>}
          </div>
      ...
      <div style={{ ...card, marginTop: 14, background: 'var(--panel-2, rgba(0,0,0,0.03))' }}>
        ...
        <p style={{ fontSize: 12.5, color: 'var(--text-dim)', lineHeight: 1.55, margin: 0 }}>
          ...
          prove the cited work exists or that the model read it. The committed range proof (Pedersen + per-bit OR-proofs) shows
          the confidence cleared the bar while hiding both the value and the gap — but it is not a calibration against
