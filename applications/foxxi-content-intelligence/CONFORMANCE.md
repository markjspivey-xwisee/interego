...
| Signed Statements (RFC 7515 JWS attachment) | **Partial** | **Inbound (LRS side) is implemented and enforced**: [`src/xapi-lrs.ts`](src/xapi-lrs.ts) — `checkStatementAttachments()` requires a `usageType=http://adlnet.gov/expapi/attachments/signature` attachment to be `application/octet-stream`, and `checkJwsSignature()` requires a well-formed 3-segment JWS whose protected header carries an `alg` in `ALLOWED_JWS_ALGS` (RS256 / RS384 / RS512 — `none` and symmetric algorithms are refused) and whose payload is a JSON serialisation of the Statement. **Outbound is not implemented**: nothing in the tree produces a signature attachment. The LTI 1.3 helper `jwsSignEs256()` is *not* the path — it emits ES256, which this LRS rejects with a 400; producing signed Statements needs an RSA-SHA2 key, which the P-256 LTI keypair cannot supply. |
...
| Technical (duration, format, version) | **Partial** | [`src/lom-sequencing.ts`](src/lom-sequencing.ts) `lomToTurtle()` §4 emits `lom:format`, `lom:size` and `lom:duration` when `LomMetadata.technical` is populated — but no caller populates it, so nothing is auto-extracted from a manifest today. (The `schema:duration` / `schema:softwareVersion` terms previously cited here appear in no source file.) |
...
| OB3 issuer (Profile) | **Partial** | Issued VCs carry `issuer` as the tenant's `did:key` string; tenant Profile name/details ride in published descriptor metadata. The normative `issuer: { id, type, name }` object shape is **already supported by the crypto layer** — `issuerId()` in [`../_shared/vc-jwt/data-integrity-jcs.ts`](../_shared/vc-jwt/data-integrity-jcs.ts) accepts both forms, and both `issueDataIntegrityProof()` and `verifyDataIntegrityProof()` route through it, so an object issuer signs, verifies and stays tamper-bound today. No substrate change is involved (there is no `VcPayload` type; the `readonly issuer: string` narrowing is application-owned). What blocks emitting it is the consumers that assume a string: `src/bbs-credentials.ts` commits `` `issuer=${credential.issuer}` `` into a BBS message through a `Record<string, unknown>` parameter the typechecker cannot police — an object would bind the constant `[object Object]` for every issuer — plus `src/credentials.ts` (Turtle IRI interpolation), `src/clr-1.ts`, `src/clr.ts` and `src/pgsl-ingestion-profiles.ts`. Each must route through `issuerId()` before the object form is emitted. |
...
| Standard requirement | Status | Where |
|---|---|---|
| Tool JWKS (RFC 7517) at `/lti/.well-known/jwks.json` | **Compliant** | [`src/lti13.ts`](src/lti13.ts) — ES256 (P-256) keypair from `FOXXI_LTI_PRIVATE_KEY_PEM`, or a fresh one per process; the `kid` hashes the public key |
...
| Resource Link Launch (LTI 1.3 §5.1.2) | **Compliant** | `POST /lti/launch` — verifies `id_token` against the platform's JWKS (RS256 or ES256), validates `iss` / `aud` / `nonce` / `exp` / `deployment_id` / `version=1.3.0` / `message_type`, then either opens the course in the SCORM engine (a launch from Foxxi's own LMS, below) or mints a 5-min HMAC-signed launch ticket and redirects to the Foxxi dashboard |
| Deep Linking 2.0 — `GET` + `POST /lti/deeplink` | **Compliant** | Content picker over the registered cmi5 courses; a signed `LtiDeepLinkingResponse` JWT (with the platform's `data` echoed) auto-posted to `deep_link_return_url`; `accept_multiple` respected |
...
| AGS — line items | **Compliant** | `GET/POST /lti/ags/lineitems`, `GET/PUT/DELETE /lti/ags/lineitems/:id` — a per-tenant line-item store kept on the tenant pod, optionally mirrored onto a platform's container; operator-only |
| Names & Roles Provisioning Service (NRPS 2.0) | **Compliant** | `GET /lti/nrps/members` — the tenant roster as a membership container, or `?members_url=` read from a platform with a Tool-signed token; both are operator-only, and an anonymous consumer call never requests a platform token |
| Multi-platform / multi-tenant registration | **Compliant** | `FOXXI_LTI_PLATFORMS` env — comma-separated `issuer\|\|client_id\|\|deployment_id\|\|jwks_url\|\|auth_login_url\|\|auth_token_url`; each registered platform gets independent verify + token paths; Foxxi's own LMS is registered in code beside them |
| Platform: OIDC authorization (LTI 1.3 §5.1.1; OAuth 2.0 Form Post Response Mode) | **Compliant** | [`src/lti-platform.ts`](src/lti-platform.ts) `GET/POST /lti/platform/auth` — checks `response_type=id_token`, `scope=openid`, the Tool's `client_id`, an exactly registered `redirect_uri` and a `nonce`; spends a one-use grant bound to the learner (`lti_message_hint`, checked against `login_hint`) that lapses in five minutes; signs an ES256 `id_token` with the LtiResourceLinkRequest claims, custom parameters and the AGS endpoint claim, and posts it by `form_post` (as JSON to a client that asks) |
| Platform: keys and configuration | **Compliant** | `GET /lti/platform/jwks.json`; `GET /lti/platform/.well-known/openid-configuration` with the LTI platform configuration |
| Platform: token endpoint (1EdTech Security Framework §4.1; RFC 7523) | **Compliant** | `POST /lti/platform/token` — verifies the Tool's `client_assertion` against the Tool's JWKS: `iss` = `sub` = the `client_id`, `aud` = this endpoint, unexpired and expiring within ten minutes, each `jti` used once (a full replay cache refuses rather than forget a live one); grants only the AGS scopes it serves |
| Platform: AGS line items, results, score publish (AGS 2.0) | **Compliant** | `GET …/lineitems`, `GET …/lineitems/:id`, `GET …/lineitems/:id/results`, `POST …/lineitems/:id/scores` under `/lti/platform/contexts/:context` — bearer and scope checked; a score only for a learner the LMS launched; the latest by `timestamp` kept (an older one answers 409); results rescaled to the line item's `scoreMaximum` |
| Platform: Deep Linking and NRPS as a Platform | **Not implemented** | the Platform launches resource links only |
