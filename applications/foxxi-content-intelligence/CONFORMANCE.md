...
| Platform: token endpoint (1EdTech Security Framework §4.1; RFC 7523) | **Compliant** | `POST /lti/platform/token` — verifies the Tool's `client_assertion` against the Tool's JWKS: `iss` = `sub` = the `client_id`, `aud` = this endpoint, unexpired, each `jti` used once; grants only the AGS scopes it serves |
