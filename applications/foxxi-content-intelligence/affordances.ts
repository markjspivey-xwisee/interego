...
export const foxxiAdminAffordances: ReadonlyArray<Affordance> = [
...

  {
    ...
    inputs: [
      ...
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON string carrying at least { agent_id, timestamp, tenant_pod_url }. May also carry learner_id / learner_pod_url / tenant_did.' },
      { name: 'tenant_pod_url', type: 'string', required: true, description: 'The pod that hosts your self-sovereign tenant membership (usually your own pod). Sign it inside _signed_payload; sent in the clear it is advisory only.' },
