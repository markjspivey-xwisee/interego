...
export const foxxiAffordances: ReadonlyArray<Affordance> = [
  ...
  {
    action: 'urn:iep:action:foxxi:cmi5-launch-signed' as IRI,
    toolName: 'foxxi.cmi5_launch_signed',
    title: 'Launch a cmi5 activity for myself',
    description: 'Launch an Assignable Unit of a cmi5 course published on this bridge, for yourself, signed by your wallet or by an agent holding your delegation (a relay connection, such as a Claude connector). The launch is for the signer and nobody else. The LMS stages LMS.LaunchData in the State resource and returns the conformant launch URL; the AU fetches its auth-token once. Its statements, and the `satisfied` the LMS records when they meet moveOn, land in your own record and are kept on your pod, where your IEEE P2997 learner record reads them. They count as experience, not as graded evidence for a credential: a cmi5 AU reports its own result. Without au_id, the first AU you have not satisfied; sequential progression is enforced (409 names the AU that comes first). Externally routed: sign_request the args, then POST the envelope.',
    method: 'POST',
    targetTemplate: '{base}/agent/cmi5/launch',
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Launch a cmi5 activity', readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
    inputs: [
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON.stringify({ agent_id, timestamp, course_id, au_id?, return_url? }): the published course, optionally the AU, optionally where the AU returns you.' },
      { name: '_signature', type: 'string', required: true, description: 'sign_request signature (secp256k1 over sha256 of _signed_payload).' },
    ],
    appliesTo: { collections: ['profiles'] },
    outputs: {
      description: 'The launch: registration, launchUrl, actor, and the AU with its moveOn. 404 for a course or AU this bridge does not publish; 409 when an earlier AU is not yet satisfied; 401 on auth failure.',
      properties: {
        registration: { type: 'string', description: 'The cmi5 registration this launch opened.' },
        launchUrl: { type: 'string', description: 'The conformant cmi5 launch URL: open it to run the AU.' },
        actor: { type: 'object', description: 'The xAPI Agent the AU records as: you.', additionalProperties: true },
        auId: { type: 'string', description: 'The AU launched.' },
        moveOn: { type: 'string', description: 'The criterion the LMS applies before recording satisfied.' },
        launchDataStaged: { type: 'boolean', description: 'LMS.LaunchData is in the State resource for the AU to read.' },
        progress: { type: 'string', description: 'Where the registration\'s moveOn state can be read.' },
        learner: { type: 'string', description: 'Who the launch is for: your wallet DID, or the person your delegation names.' },
      },
    },
  },
  {
    action: 'urn:iep:action:foxxi:lti-launch-signed' as IRI,
    toolName: 'foxxi.lti_launch_signed',
    title: 'Launch a course from Foxxi\'s own LMS over LTI 1.3',
    description: 'Launch a course this bridge\'s SCORM engine grades from Foxxi\'s own LMS, the LTI 1.3 Platform this bridge runs beside its Tool, for yourself, signed by your wallet or by an agent holding your delegation (a relay connection, such as a Claude connector). The LMS knows who you are from the signature and returns an initiation URL, used once within five minutes. From there it is a standard launch over HTTP: the Tool\'s OIDC login, the LMS\'s authorization, a signed id_token posted to the Tool, then the course. A browser does all of it; a client without one follows the redirects, asks the authorization step for JSON, posts the form it returns, and plays the course as JSON. When the attempt ends the SCORM engine\'s outcome is in your record, where your IEEE P2997 learner record reads it, and the Tool posts your grade to the LMS gradebook over Assignment and Grade Services (foxxi.lti_gradebook_signed reads it). Externally routed: sign_request the args, then POST the envelope.',
    method: 'POST',
    targetTemplate: '{base}/agent/lti/launch',
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Launch a course over LTI', readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
    inputs: [
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON.stringify({ agent_id, timestamp, course_id }): a course this bridge\'s SCORM engine grades.' },
      { name: '_signature', type: 'string', required: true, description: 'sign_request signature (secp256k1 over sha256 of _signed_payload).' },
    ],
    appliesTo: { collections: ['profiles'] },
    outputs: {
      description: 'The launch: the initiation URL, the LMS course context, the resource link and its gradebook line item. 404 for a course this bridge does not grade; 429 when rate limited; 401 on auth failure.',
      properties: {
        initiationUrl: { type: 'string', description: 'The OIDC third-party-initiated login at the Tool: open it once, within five minutes.' },
        expiresAt: { type: 'string', description: 'When the launch can no longer be started.' },
        platform: { type: 'object', description: 'The LMS: its issuer and its OpenID configuration.', additionalProperties: true },
        lineItem: { type: 'object', description: 'The gradebook column the grade is posted to.', additionalProperties: true },
        learner: { type: 'string', description: 'Who the launch is for: your wallet DID, or the person your delegation names. It is the LTI sub.' },
      },
    },
  },
  {
    action: 'urn:iep:action:foxxi:lti-gradebook-signed' as IRI,
    toolName: 'foxxi.lti_gradebook_signed',
    title: 'Read my grades in Foxxi\'s own LMS',
    description: 'Your row of the gradebook in Foxxi\'s own LMS, signed by your wallet or by an agent holding your delegation: every course in the LMS course context, and the grade the Tool posted for you over LTI Assignment and Grade Services, or null where there is none yet. Only your own row; who you are comes from the signature. Externally routed: sign_request the args, then POST the envelope.',
    method: 'POST',
    targetTemplate: '{base}/agent/lti/gradebook',
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Read my LMS grades', readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON.stringify({ agent_id, timestamp }).' },
      { name: '_signature', type: 'string', required: true, description: 'sign_request signature (secp256k1 over sha256 of _signed_payload).' },
    ],
    appliesTo: { collections: ['profiles'] },
    outputs: {
      description: 'Your gradebook rows. 401 on auth failure.',
      properties: {
        rows: { type: 'array', description: 'One per course: its line item and your result, rescaled to the line item\'s maximum, or null.', items: { type: 'object', additionalProperties: true } },
        learner: { type: 'string', description: 'Whose rows these are: the LTI sub the LMS knows you by.' },
      },
    },
  },
  {
