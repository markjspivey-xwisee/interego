 ...
 *
 * Action IRIs use the urn:iep:action:foxxi:<verb> convention. Targets
...
export const foxxiAffordances: ReadonlyArray<Affordance> = [
  ...
  {
    action: 'urn:iep:action:foxxi:earned-credentials' as IRI,
    toolName: 'foxxi.earned_credentials',
    title: 'Where every assigned course stands',
    ...
    method: 'POST',
    targetTemplate: '{base}/foxxi/earned_credentials',
    annotations: { title: 'Where every assigned course stands', readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: 'learner_did', type: 'string', required: false, description: 'The learner\'s WebID; omitted, the caller. An admin may name any learner.' },
      { name: 'learner_pod_url', type: 'string', required: false, description: 'The learner\'s pod when it is not derived from their identity; bounded to the pod space this deployment reads.' },
      { name: 'tenant_pod_url', type: 'string', required: false, description: 'The tenant whose catalog and assignments apply; omitted, the configured tenant.' },
      { name: 'audience_tags', type: 'array', required: false, description: 'Audience tags to resolve the assignments with, for a learner the tenant directory does not list.', itemType: 'string' },
    ],
    appliesTo: { collections: ['profiles'] },
    outputs: {
      description: 'The learner\'s standing per assigned course, the claimable ones first, with the credential held or lapsed and the evidence counts, plus an ABAC accessDecision trace.',
      properties: {
        learner: { type: 'string', description: 'The learner the standings are about.' },
        learnerPodUrl: { type: 'string', description: 'The pod the record and the wallet were read from.' },
        standings: { type: 'array', description: 'Per course: courseId, courseTitle, state (credentialed | claimable | in-progress | not-started), dueAt, credential, lapsed, statements, mastery, and for a claimable course the claim call.', items: { type: 'object', additionalProperties: true } },
        held: { type: 'integer', description: 'How many credentials the wallet holds.' },
        accessDecision: { type: 'object', description: 'ABAC trace: caller role + applied policies + the decision.', additionalProperties: true },
      },
    },
  },
  {
    action: 'urn:iep:action:foxxi:claim-credential' as IRI,
    toolName: 'foxxi.claim_credential',
    title: 'Claim a credential the record has earned',
    description: 'The tenant issues an Open Badges 3.0 completion credential for a catalog course only from the learner\'s own record: a passed, mastered, satisfied or waived xAPI statement about the course, with success not false and any score at or above the course\'s threshold, and graded by this bridge itself: the SCORM engine\'s completions carry the bridge\'s grading tag, so a statement the learner recorded is in the record but does not earn a credential. The credential names that evidence, is signed by the tenant\'s issuer key, is valid for a year, and is written to the learner\'s pod wallet; a credentialed statement joins their record. Already held and in force: the held credential comes back and nothing is issued. Not earned: a 409 refusal saying what statement would earn it. A learner claims for themselves; an admin may claim for a learner.',
    method: 'POST',
    targetTemplate: '{base}/foxxi/claim_credential',
    annotations: { title: 'Claim a credential the record has earned', readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: 'course_id', type: 'string', required: true, description: 'The tenant catalog course id the credential is for.' },
      { name: 'learner_did', type: 'string', required: false, description: 'The learner\'s WebID; omitted, the caller. An admin may name any learner.' },
      { name: 'learner_pod_url', type: 'string', required: false, description: 'The learner\'s pod when it is not derived from their identity; bounded to the pod space this deployment reads.' },
      { name: 'tenant_pod_url', type: 'string', required: false, description: 'The tenant whose catalog applies and whose issuer signs; omitted, the configured tenant.' },
    ],
    appliesTo: { collections: ['profiles'] },
    outputs: {
      description: 'The decision: issued (credentialId, descriptorUrl, graphUrl, issuer, validUntil, the evidence statements, the signed vc, the credentialed statement id) or already-held (the credential in force); a refusal with status 409 when the record has not earned it, saying what would.',
      properties: {
        decision: { type: 'string', description: 'issued | already-held', enum: ['issued', 'already-held'] },
        courseId: { type: 'string', description: 'The catalog course the credential is for.' },
        credentialId: { type: 'string', description: 'The credential\'s id: its own dereferenceable descriptor location on the learner\'s pod.' },
        descriptorUrl: { type: 'string', description: 'Where the credential descriptor was published in the wallet.' },
        validUntil: { type: 'string', description: 'When the issued credential expires (ISO 8601).' },
        evidence: { type: 'array', description: 'The mastery statements the credential names: id, verb, object, timestamp, scoreScaled, success.', items: { type: 'object', additionalProperties: true } },
        credential: { type: 'object', description: 'For already-held: the credential in force (id, descriptorUrl, achievementId, issuer, validFrom, validUntil, verified).', additionalProperties: true },
        vc: { type: 'object', description: 'The signed Open Badges 3.0 credential as issued.', additionalProperties: true },
        accessDecision: { type: 'object', description: 'ABAC trace: caller role + applied policies + the decision.', additionalProperties: true },
      },
    },
  },
  {
    ...
    outputs: {
      ...
      properties: {
        valid: { type: 'boolean', description: 'Every check passed.' },
        checks: { type: 'object', description: 'signature, notExpired, inForce, issuerTrusted, subjectBound: each true or false.', additionalProperties: true },
        issuer: { type: 'string', description: 'The stated issuer DID.' },
        subject: { type: 'string', description: 'The credential subject.' },
  ...
  {
    action: 'urn:iep:action:foxxi:earned-credentials-signed' as IRI,
    toolName: 'foxxi.earned_credentials_signed',
    title: 'Where my courses stand, signed as myself',
    description: 'foxxi.earned_credentials for the courses this bridge\'s SCORM engine grades, signed the way the engine\'s own routes are: by your wallet, or by an agent holding your delegation (a relay connection, such as a Claude connector). Reads your own record and wallet — the pod the signature derives, never one you name — and reports each course your record is about, and any you list in course_ids: credentialed, claimable (with the claim to make), in progress, or not started. Externally routed: sign_request the args, then POST the envelope.',
    method: 'POST',
    targetTemplate: '{base}/agent/credentials/earned',
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Where my courses stand', readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON.stringify({ agent_id, timestamp, course_ids? }).' },
      { name: '_signature', type: 'string', required: true, description: 'sign_request signature (secp256k1 over sha256 of _signed_payload).' },
    ],
    appliesTo: { collections: ['profiles'] },
    outputs: {
      description: 'Your standing per course, the claimable ones first, each claimable one with the claim to make. 401 on auth failure.',
      properties: {
        learner: { type: 'string', description: 'Who the standings are about: your wallet DID, or the person your delegation names.' },
        via: { type: 'string', description: 'The agent that signed for you, when it was a delegation.' },
        learnerPodUrl: { type: 'string', description: 'The pod the record and the wallet were read from.' },
        standings: { type: 'array', description: 'Per course: courseId, courseTitle, state (credentialed | claimable | in-progress | not-started), credential, lapsed, statements, mastery, and for a claimable course the claim (affordance, target, payload).', items: { type: 'object', additionalProperties: true } },
        held: { type: 'integer', description: 'How many credentials the wallet holds.' },
      },
    },
  },
  {
    action: 'urn:iep:action:foxxi:claim-credential-signed' as IRI,
    toolName: 'foxxi.claim_credential_signed',
    title: 'Claim a credential the record earned, signed as myself',
    description: 'foxxi.claim_credential for a course this bridge\'s SCORM engine grades, signed by your wallet or by an agent holding your delegation (a relay connection, such as a Claude connector). The tenant issues an Open Badges 3.0 completion credential only from your own record — a passed, mastered, satisfied or waived statement about the course with success not false and any score at or above its threshold, graded by this bridge (the engine\'s results carry its grading tag; a statement you wrote yourself is in the record but is not evidence). The credential names that evidence and the course\'s author, is signed by the tenant\'s issuer key, is valid for a year, and is written to your pod\'s wallet; a delegated claim is issued to the person the delegation names. Already held and in force: the held credential comes back. Not earned: 409 saying what would earn it. Externally routed: sign_request the args, then POST the envelope.',
    method: 'POST',
    targetTemplate: '{base}/agent/credentials/claim',
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Claim an earned credential', readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON.stringify({ agent_id, timestamp, course_id }).' },
      { name: '_signature', type: 'string', required: true, description: 'sign_request signature (secp256k1 over sha256 of _signed_payload).' },
    ],
    appliesTo: { collections: ['profiles'] },
    outputs: {
      description: 'issued (credentialId, descriptorUrl, graphUrl, issuer, validUntil, evidence, courseAuthor, vc) or already-held (credential). 409 when your record has not earned it, saying what would; 404 for a course the engine does not play; 401 on auth failure.',
      properties: {
        decision: { type: 'string', description: 'issued | already-held', enum: ['issued', 'already-held'] },
        courseId: { type: 'string', description: 'The course the credential is for.' },
        credentialId: { type: 'string', description: 'The credential\'s id: its own descriptor location on your pod.' },
        descriptorUrl: { type: 'string', description: 'Where the credential descriptor was published in your wallet.' },
        learner: { type: 'string', description: 'Who the credential names: your wallet DID, or the person your delegation names.' },
        courseAuthor: { type: 'string', description: 'Who authored the course, as the credential\'s criteria also say.' },
        vc: { type: 'object', description: 'The signed Open Badges 3.0 credential as issued.', additionalProperties: true },
        credential: { type: 'object', description: 'For already-held: the credential in force.', additionalProperties: true },
      },
    },
  },
  {
  ...
  {
    action: 'urn:iep:action:foxxi:ask-course-question' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:ask-course-question-agentic' as IRI,
    ...
    appliesTo: { collections: ['courses'] },
    outputs: {
      description: 'Agentic-RAG result: federated retrieval scaffold + LLM-synthesised answer + full descriptor trace (fxa:LearnerQuestionEvent Asserted → fxa:RetrievalActivity Hypothetical → fxa:LlmCompletion Hypothetical → fxa:CitedAnswer Asserted, with iep:supersedes chains). When no LLM key is configured, returns the retrieval scaffold + trace only (use foxxi.retrieve_course_context for the explicit no-LLM path). Surfaces a rate-limit error when bridge-env key is used and per-IP rate cap is hit.',
      properties: {
        ...
        trace: { type: 'array', description: 'Ordered Interego descriptor trace; one entry per agent step (event type + descriptor IRI + modal status + iep:supersedes).', items: { type: 'object', additionalProperties: true } },
  ...
  {
    action: 'urn:iep:action:foxxi:retrieve-course-context' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:extend-standards' as IRI,
    ...
    outputs: {
      description: 'ProposedExtension — the new term IRI, a conformant artifact (xAPI Profile doc or Turtle term), a publishable iep:StandardsExtension descriptor, and _guidance (in-flow performance support).',
      properties: {
  ...
  // ─────────────────────────────────────────────────────────────────────
  //  Externally-routed capability affordances (hand-coded /agent/* routes)
  //
  //  Each entry below is `externallyRouted: true` — DECLARED here so the
  //  capability is discoverable in the /affordances manifest, /mcp tools/list,
  //  and the entry point, while the route body stays in bridge/server.ts (it
  //  carries bespoke signed-delegation auth or rate-limited open access). The
  //  bridge does NOT auto-register a route or require a handler for these; the
  //  targetTemplate PRESERVES the exact existing path so the hand-coded route
  //  keeps serving it. CI gate: tools/check-foxxi-affordances.mjs.
  // ─────────────────────────────────────────────────────────────────────

  {
    ...
    outputs: {
      ...
      properties: {
        ok: { type: 'boolean' },
        reviewedAs: { type: 'string', description: 'Caller DID the record was assembled for.' },
        authMode: { type: 'string', enum: ['direct', 'delegated'], description: 'How the caller identity was bound.' },
        self: { type: 'boolean', description: 'True when the subject is the caller.' },
        subject: { type: 'object', description: 'Subject envelope: { did, podUrl, label, kind, lensTenant, statementCount, statementSource, latticeStatements }.', additionalProperties: true },
        elr: { type: 'object', description: 'The assembled IEEE P2997 Enterprise Learner Record.', additionalProperties: true },
        clr: { type: 'object', description: 'Verified 1EdTech CLR wallet (present unless include_clr=false).', additionalProperties: true },
      },
      required: ['ok'],
    },
  },
  ...
  {
    action: 'urn:iep:action:foxxi:issue-credential' as IRI,
    ...
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Issue a competency credential', readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON.stringify({ agent_id, timestamp, recipient_did, recipient_pod_url?, recipient_name?, competency_name, competency_id?, competency_framework?, achievement_description?, criterion?, evidence?, justified_by? }).' },
      { name: '_signature', type: 'string', required: true, description: 'secp256k1 over sha256:<hex(sha256(_signed_payload))> by the wallet matching agent_id (relay sign_request).' },
    ],
    appliesTo: { collections: ['profiles'] },
    outputs: {
      description: 'The issued OB3/W3C VC, its issuer + recipient, and the shared-lattice composition. 503 when FOXXI_ISSUER_KEY_SEED is unset; 401 on auth failure.',
      properties: {
        ok: { type: 'boolean' },
        issuedBy: { type: 'string', description: 'Caller DID acting as the issuing authority.' },
        issuerDid: { type: 'string', description: 'Per-creator custodied issuer DID on the VC.' },
        credentialId: { type: 'string', description: 'VC id.' },
        recipient: { type: 'object', description: '{ did, podUrl }.', additionalProperties: true },
        competency: { type: 'object', description: '{ id, name }.', additionalProperties: true },
        descriptorUrl: { type: 'string', description: 'Published credential descriptor URL on the recipient pod.' },
        vc: { type: 'object', description: 'The full signed Verifiable Credential.', additionalProperties: true },
      },
      required: ['ok'],
    },
  },
  ...
  {
    action: 'urn:iep:action:foxxi:verify-extension' as IRI,
    toolName: 'foxxi.verify_extension',
    title: 'Independently verify a subject extended a standard (issuer due diligence)',
    ...
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Independently verify an extension', readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON.stringify({ agent_id, timestamp, subject_did, name?, kind?, definition?, subject_pod_url? }). kind defaults to XapiContextExtension; name triggers the shape-conformance check.' },
      { name: '_signature', type: 'string', required: true, description: 'sign_request signature (secp256k1 over sha256 of _signed_payload).' },
    ],
    appliesTo: { collections: ['profiles'] },
    outputs: {
      description: 'The verification verdict + per-check evidence + (on success) the verification holon URI.',
      properties: {
        ok: { type: 'boolean' },
        verifiedBy: { type: 'string', description: 'Caller DID acting as verifier.' },
        subject: { type: 'object', description: '{ did, podUrl, statementCount }.', additionalProperties: true },
        verified: { type: 'boolean', description: 'Overall verdict (independently graded ∧ performance recorded ∧ shape conformant).' },
        checks: { type: 'object', description: '{ independentlyGraded, gradedScore, performanceRecorded, selfAttestedPerformance, shapeConformant }.', additionalProperties: true },
        verificationHolonUri: { type: 'string', description: 'Dereferenceable iep:Verification holon composed on the verifier\'s lattice (present only when verified).' },
        evidence: { type: 'array', description: 'Statement ids that grounded the verdict.', items: { type: 'string' } },
        note: { type: 'string', description: 'Human-readable verdict narrative.' },
      },
      required: ['ok'],
    },
  },

  {
    ...
    method: 'POST',
    // ★ Points at the GROUNDED implementation. This manifest entry used to target
    // {base}/agent/prove-competency, which minted a credential from claims supplied
    // in the request — so the DISCOVERABLE path was the ungrounded one, while the
    // route that checks the record sat unadvertised. An agent that follows published
    // affordances (which is the whole contract of this system) got the weaker one.
    ...
    inputs: [
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON.stringify({ agent_id, timestamp, tenant_pod_url, competency_name, learner_did?, learner_name?, course_id?, reveal_paths?, presentation_context? }). reveal_paths is an array of claim paths to disclose (default ["issuer","achievement.name","achievement.proficiencyLevel"]); any path the credential does not carry is dropped and reported back in unknownRevealPaths. presentation_context binds the proof to one occasion — a verifier MUST echo it back or the proof will not verify. NOTE: score and proficiency are NOT inputs; both are derived from the holder\'s record.' },
      { name: '_signature', type: 'string', required: true, description: 'sign_request signature by the holder (subject of the credential).' },
    ],
    appliesTo: { collections: ['profiles'] },
    outputs: {
      description: 'The serialized BBS+ selective-disclosure presentation + which claims were revealed vs cryptographically hidden + the record evidence the proficiency was derived from. Returns { error } when the competency is not asserted in the holder\'s record, or when FOXXI_ISSUER_KEY_SEED is unset.',
      properties: {
        verified: { type: 'boolean', description: 'Whether the derived proof verifies (checked at derivation time).' },
        issuerDid: { type: 'string', description: 'Issuer DID that signed the credential.' },
        competencyName: { type: 'string', description: 'The competency proved.' },
        totalClaims: { type: 'integer', description: 'Number of signed claims the BBS+ signature commits to.' },
        revealedClaims: { type: 'array', description: 'Disclosed { path, value } pairs — what a verifier learns.', items: { type: 'object', additionalProperties: true } },
        hiddenClaimCount: { type: 'integer', description: 'How many claims stayed behind the zero-knowledge proof.' },
        hiddenClaimPaths: { type: 'array', description: 'Claim paths cryptographically withheld.', items: { type: 'string' } },
        unknownRevealPaths: { type: 'array', description: 'Requested reveal paths the credential does not carry, and which were therefore dropped. Non-empty here means you disclosed less than you intended.', items: { type: 'string' } },
        presentation: { type: 'object', description: 'Serialized BBS+ presentation (base64 proof + disclosed messages + issuer public key + presentationContext) — feed directly to foxxi.verify_presentation.', additionalProperties: true },
        proofB64: { type: 'string', description: 'Proof bytes alone. NOT sufficient to verify — use `presentation`.' },
        derivedFromRecord: { type: 'object', description: 'The rollup this proof was grounded in: Dreyfus proficiency, confidence, the rule that produced it, and the competency IRI.', additionalProperties: true },
      },
      required: ['verified'],
    },
  },
  ...
  {
    action: 'urn:iep:action:foxxi:self-assert-competency' as IRI,
    toolName: 'foxxi.self_assert_competency',
    title: 'Self-assert a competency with selective disclosure (SelfAsserted — not an authority attestation)',
    description: 'Mint a BBS+ credential about YOURSELF and derive a selective-disclosure presentation from it. The issuer is always the authenticated signer: this route will not issue under another party\'s identity, and it does not read score or proficiency from the request, so the claim carries the floor of the scale (Novice). What a verifying proof establishes here is INTEGRITY — the holder said this and cannot have altered it since — and not merit. Results carry selfIssued, trustLevel=SelfAsserted and claimsGroundedInRecord=false so a relying party can weigh them without knowing this route. For a proof grounded in a real learning record, and a proficiency derived from the published rollup, use foxxi.prove_competency instead. Externally routed: sign_request the args, then POST the envelope.',
    method: 'POST',
    targetTemplate: '{base}/agent/prove-competency',
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Self-assert a competency (BBS+)', readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON.stringify({ agent_id, timestamp, competency_name, reveal? }). reveal is an array of claim paths to disclose (default ["issuer","achievement.name","achievement.proficiencyLevel"]). issuer_did, score and proficiency are NOT accepted — naming an issuer other than the signer returns 403.' },
      { name: '_signature', type: 'string', required: true, description: 'sign_request signature. The recovered signer is both the holder AND the issuer.' },
    ],
    appliesTo: { collections: ['profiles'] },
    outputs: {
      description: 'The serialized BBS+ presentation plus explicit provenance labels. 403 when issuer_did names a party other than the signer; 503 when FOXXI_ISSUER_KEY_SEED is unset.',
      properties: {
        ok: { type: 'boolean' },
        holder: { type: 'string', description: 'Holder DID — also the issuer.' },
        issuerDid: { type: 'string', description: 'Issuer key, derived from the authenticated signer.' },
        selfIssued: { type: 'boolean', description: 'Always true on this route.' },
        trustLevel: { type: 'string', description: 'Always "SelfAsserted".' },
        claimsGroundedInRecord: { type: 'boolean', description: 'Always false — the claims came from the request, not a learner record.' },
        groundedAlternative: { type: 'string', description: 'The affordance to use when the claim must be backed by evidence.' },
        revealed: { type: 'array', description: 'Disclosed { path, value } pairs.', items: { type: 'object', additionalProperties: true } },
        hiddenPaths: { type: 'array', description: 'Claim paths cryptographically withheld.', items: { type: 'string' } },
        presentation: { type: 'object', description: 'Serialized BBS+ presentation for foxxi.verify_presentation.', additionalProperties: true },
      },
      required: ['ok'],
    },
  },

  {
    ...
    inputs: [
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON.stringify({ agent_id, timestamp, presentation: { proof, disclosedIndexes, disclosedMessages, issuerPublicKey, issuerDid } }) — the presentation from foxxi.prove_competency.' },
      { name: '_signature', type: 'string', required: true, description: 'sign_request signature by the verifier.' },
    ],
    appliesTo: { collections: ['profiles'] },
    outputs: {
      description: 'The verification result + which claims the verifier learned. 400 when the presentation envelope is malformed.',
      properties: {
        ok: { type: 'boolean' },
        verifiedBy: { type: 'string', description: 'Verifier DID.' },
        issuerDid: { type: 'string', description: 'Issuer DID the proof attests.' },
        verified: { type: 'boolean', description: 'Whether the BBS+ proof verified.' },
        reason: { type: 'string', description: 'Why the proof did/did not verify.' },
        disclosed: { type: 'array', description: 'The claims the verifier learned.', items: { type: 'object', additionalProperties: true } },
        learned: { type: 'integer', description: 'Count of disclosed claims.' },
      },
      required: ['ok'],
    },
  },
  ...
  {
    action: 'urn:iep:action:foxxi:void-credential' as IRI,
    ...
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Void a credential', readOnlyHint: false, destructiveHint: true, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON.stringify({ agent_id, timestamp, descriptor_url, subject_pod_url? }) — descriptor_url = a CLR entry sourceDescriptor under your own pod.' },
    ...
    appliesTo: { collections: ['profiles'] },
    outputs: {
      description: 'The deletion result + manifest rebuild summary. 403 when the descriptor_url is not under your own pod.',
      properties: {
        ok: { type: 'boolean' },
        voidedBy: { type: 'string', description: 'Caller DID.' },
        descriptor: { type: 'string', description: 'The voided descriptor URL.' },
        deletions: { type: 'object', description: 'Per-resource HTTP delete status (path → status).', additionalProperties: true },
        manifest: { type: 'object', description: 'Manifest rebuild summary { written, scanned } (or "rebuild-failed").', additionalProperties: true },
      },
      required: ['ok'],
    },
  },
  ...
  {
    action: 'urn:iep:action:foxxi:publish-encryption-key' as IRI,
    ...
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Publish your encryption key', readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON.stringify({ agent_id, timestamp, public_key, subject_pod_url? }) — public_key is your base64 X25519 (Curve25519) public key (algorithm X25519-XSalsa20-Poly1305).' },
      { name: '_signature', type: 'string', required: true, description: 'secp256k1 over sha256:<hex(sha256(_signed_payload))> by the wallet matching agent_id.' },
    ],
    appliesTo: { collections: ['profiles'] },
    outputs: {
      description: 'Where the public key was published + the algorithm. 400 when public_key is missing.',
      properties: {
        ok: { type: 'boolean' },
        published: { type: 'string', description: 'URL of the published <pod>/keys/encryption.json.' },
        owner: { type: 'string', description: 'Caller DID (owner of the key).' },
        algorithm: { type: 'string', description: 'X25519-XSalsa20-Poly1305.' },
      },
      required: ['ok'],
    },
  },
  ...
  {
    action: 'urn:iep:action:foxxi:course-analyze' as IRI,
    toolName: 'foxxi.course_analyze',
    title: 'Analyze a SCORM package: fingerprint the authoring tool + build a course KG',
    description: 'Fingerprint WHICH authoring tool produced a SCORM package (from its imsmanifest.xml + file list/contents), build the concept/slide knowledge-graph, and compose it into a per-course PGSL lattice holon (dereferenceable + interrogable). Open + rate-limited per IP (it composes a holon on success). Externally routed (rate-limited open access, no signature).',
    method: 'POST',
    targetTemplate: '{base}/agent/course/analyze',
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Analyze a SCORM course', readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: 'manifestXml', type: 'string', required: true, description: 'imsmanifest.xml text (must contain <manifest...; max 2MB).' },
      { name: 'fileList', type: 'array', required: false, description: 'Optional list of file paths in the package (capped at 5000) — strengthens fingerprinting.' },
      { name: 'fileText', type: 'object', required: false, description: 'Optional map of filename → extracted text (capped) for concept extraction.' },
      { name: 'fileContents', type: 'object', required: false, description: 'Optional map of filename → raw content (capped) for fingerprint signals.' },
    ],
    appliesTo: { collections: ['courses'] },
    outputs: {
      description: 'The authoring-tool fingerprint, the parsed course structure + concept/slide course graph, and the composed course-KG holon. 400 on missing/invalid manifest; 413 when too large; 429 on rate limit.',
      properties: {
        ok: { type: 'boolean' },
        fingerprint: { type: 'object', description: 'Authoring-tool fingerprint (tool, vendor, confidence, signals, standard, candidates).', additionalProperties: true },
        structure: { type: 'object', description: 'Parsed course structure (courseId, etc.).', additionalProperties: true },
        course: { type: 'object', description: 'The FoxxiAgenticCourse (concepts + slides) usable by /agent/course/ask.', additionalProperties: true },
        courseKg: { type: 'object', description: 'Composed course-KG holon { label, holonUri, descriptorUrl, agentDid, reusedNodes, newNodes, stats }.', additionalProperties: true },
      },
      required: ['ok'],
    },
  },
  ...
  {
    action: 'urn:iep:action:foxxi:course-analyze-authored' as IRI,
    toolName: 'foxxi.course_analyze_authored',
    title: 'Analyze an agent-authored course (cryptographic provenance fingerprint)',
    description: 'Analyze a course an agent authored via /agent/scorm/author. Because the structured course + signed authoredBy provenance is known, the "fingerprint" reports that ground truth (a Foxxi agent authored it) rather than sniffing a third-party tool. Resolves the course from the in-memory cache → the author\'s PGSL lattice → legacy pod RDF. Same response shape as foxxi.course_analyze. Externally routed (rate-limited open access).',
    method: 'POST',
    targetTemplate: '{base}/agent/course/analyze-authored',
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Analyze an agent-authored course', readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: 'courseId', type: 'string', required: true, description: 'The agent-authored course id (from /agent/scorm/author).' },
      { name: 'author_did', type: 'string', required: false, description: 'Author DID — used to resolve the course from the author\'s pod/lattice when not cached.' },
      { name: 'course_pod', type: 'string', required: false, description: 'Explicit author pod URL (defaults to the author_did\'s resolved pod).' },
    ],
    appliesTo: { collections: ['courses'] },
    outputs: {
      description: 'The provenance fingerprint + parsed course + composed course-KG holon (authored:true). 404 when the agent-authored course is not found; 429 on rate limit.',
      properties: {
        ok: { type: 'boolean' },
        fingerprint: { type: 'object', description: 'Provenance fingerprint (tool: "Foxxi (agent-authored)", confidence 1, signed authoredBy signal).', additionalProperties: true },
        structure: { type: 'object', additionalProperties: true },
        course: { type: 'object', description: 'The FoxxiAgenticCourse.', additionalProperties: true },
        courseKg: { type: 'object', description: 'Composed course-KG holon.', additionalProperties: true },
        authored: { type: 'boolean', description: 'Always true for this route.' },
      },
      required: ['ok'],
    },
  },
  ...
  {
    action: 'urn:iep:action:foxxi:course-ask' as IRI,
    toolName: 'foxxi.course_ask',
    title: 'Ask a role-framed, grounded question about an analyzed course',
    description: 'Grounded Q&A over a FoxxiAgenticCourse (from /agent/course/analyze*). A role (author / performance-manager / assessor / meta / learner) sets the lens; the answer stays grounded in the course knowledge-graph. Without llm_api_key it returns the retrieval scaffold (rate-limited; the caller\'s own LLM synthesises); with a BYOK key it runs agentic RAG synthesis. honest `grounded` flag (true only on a real graph hit). Externally routed (rate-limited; BYOK exempt).',
    method: 'POST',
    targetTemplate: '{base}/agent/course/ask',
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Ask about a course', readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: 'course', type: 'object', required: true, description: 'The FoxxiAgenticCourse (must have concepts[] + slides[]) from /agent/course/analyze*.' },
      { name: 'question', type: 'string', required: true, description: 'Natural-language question.' },
      { name: 'role', type: 'string', required: false, description: 'Lens for the answer: author | performance-manager | assessor | meta | learner.' },
      { name: 'learnerActivity', type: 'string', required: false, description: 'Optional fenced learner-activity text the role can reference (treated as untrusted data).' },
      { name: 'llm_api_key', type: 'string', required: false, description: 'BYOK Anthropic key — when present, runs LLM synthesis (rate-limit exempt); when absent, returns the retrieval scaffold for the caller\'s own LLM.' },
      { name: 'history', type: 'array', required: false, description: 'Prior conversation turns for multi-turn Q&A.' },
      { name: 'learnerDid', type: 'string', required: false, description: 'Asking learner DID (recorded; defaults to urn:foxxi:demo:asker).' },
    ],
    appliesTo: { collections: ['courses'] },
    outputs: {
      description: 'The retrieval scaffold (and, with BYOK, the synthesised answer) + the honest grounding signal. 400 on missing course/question; 429 on rate limit (non-BYOK).',
      properties: {
        ok: { type: 'boolean' },
        role: { type: 'string', description: 'Echo of the role lens.' },
        grounded: { type: 'boolean', description: 'True only on a real graph hit (retrievalKind=graph with seed concepts).' },
        retrievalKind: { type: 'string', description: 'graph | fallback — how citations were sourced.' },
        answer: { type: 'string', description: 'LLM-synthesised answer (only when llm_api_key supplied).' },
        retrieval: { type: 'object', description: 'Retrieval scaffold (seed concepts, cited slides, etc.).', additionalProperties: true },
      },
      required: ['ok'],
    },
  },
  ...
  {
    action: 'urn:iep:action:foxxi:course-skill' as IRI,
    toolName: 'foxxi.course_skill',
    title: 'Project a course to a SKILL.md an agent can load',
    description: 'Pure projection: turn a FoxxiAgenticCourse (from /agent/course/analyze*) into a SKILL.md document an agent can load — carrying the course provenance (tool, authoredBy, holonUri, courseId). No pod write, no signing. The inverse of /agent/course/analyze-skill. Externally routed.',
    method: 'POST',
    targetTemplate: '{base}/agent/course/skill',
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Course → SKILL.md', readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    inputs: [
      { name: 'course', type: 'object', required: true, description: 'The FoxxiAgenticCourse (must have concepts[] + slides[]).' },
      { name: 'tool', type: 'string', required: false, description: 'Provenance: authoring tool name to record in the skill.' },
      { name: 'authoredBy', type: 'string', required: false, description: 'Provenance: author DID.' },
      { name: 'holonUri', type: 'string', required: false, description: 'Provenance: the course holon URI.' },
    ],
    appliesTo: { collections: ['courses'] },
    outputs: {
      description: 'The generated SKILL.md text + its name. 400 on missing course.',
      properties: {
        ok: { type: 'boolean' },
        skillMd: { type: 'string', description: 'The generated SKILL.md document.' },
  ...
  {
    action: 'urn:iep:action:foxxi:course-analyze-skill' as IRI,
    toolName: 'foxxi.course_analyze_skill',
    title: 'Ingest a SKILL.md as a course knowledge-graph',
    description: 'Ingest an agent skill (SKILL.md string) as a course: parse it, report the agent-skill provenance fingerprint (ground truth — a skills.md, not a SCORM tool), and compose it into a PGSL course-KG holon so it can be interrogated, chatted with, assessed, and credentialed like any course. The inverse of /agent/course/skill. Externally routed (rate-limited open access).',
    method: 'POST',
    targetTemplate: '{base}/agent/course/analyze-skill',
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'SKILL.md → course KG', readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: 'skillMd', type: 'string', required: true, description: 'A SKILL.md string (frontmatter + body or ## sections; max 2MB).' },
    ],
    appliesTo: { collections: ['courses'] },
    outputs: {
      description: 'The agent-skill fingerprint + parsed course + composed course-KG holon (fromSkill:true). 400 on empty/unparseable skillMd; 413 when too large; 429 on rate limit.',
      properties: {
        ok: { type: 'boolean' },
        fingerprint: { type: 'object', description: 'Agent-skill provenance fingerprint.', additionalProperties: true },
        structure: { type: 'object', additionalProperties: true },
        course: { type: 'object', description: 'The FoxxiAgenticCourse derived from the skill.', additionalProperties: true },
        courseKg: { type: 'object', description: 'Composed course-KG holon.', additionalProperties: true },
        fromSkill: { type: 'boolean', description: 'Always true for this route.' },
      },
      required: ['ok'],
    },
  },
  ...
  {
    action: 'urn:iep:action:foxxi:skill-affordance' as IRI,
    toolName: 'foxxi.skill_affordance',
    title: 'Translate a SKILL.md to a typed iep:Affordance descriptor graph (round-trip)',
    description: 'The STRICT SKILL.md ⇄ iep:Affordance translator (core @interego/skills bridge): translate a SKILL.md into a real iep:Affordance ContextDescriptor graph (subject typed iep:Affordance, ieh:Affordance, hydra:Operation, dcat:Distribution) and round-trip it back to a SKILL.md to demonstrate the lossless markdown-carrier ⇄ typed-affordance translation. Pure translation: no pod write, no signing — the authoring DID rides in PROV provenance only. Externally routed.',
    method: 'POST',
    targetTemplate: '{base}/agent/skill/affordance',
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'SKILL.md ⇄ iep:Affordance', readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    inputs: [
      { name: 'skillMd', type: 'string', required: true, description: 'A SKILL.md string to translate (max 2MB).' },
      { name: 'agentDid', type: 'string', required: false, description: 'Authoring DID recorded in PROV provenance (must start with did:; defaults to the zero address).' },
    ],
    outputs: {
      description: 'The typed affordance descriptor graph + the round-tripped SKILL.md + validation. 400 on empty/unparseable skillMd; 413 when too large.',
      properties: {
        ok: { type: 'boolean' },
        skillIri: { type: 'string', description: 'IRI of the translated iep:Affordance subject.' },
        graphIri: { type: 'string', description: 'IRI of the descriptor graph.' },
        graphContent: { type: 'object', description: 'The iep:Affordance ContextDescriptor graph.', additionalProperties: true },
        roundTripMd: { type: 'string', description: 'The SKILL.md regenerated from the graph (round-trip proof).' },
        atomIris: { type: 'object', description: 'Map of atom name → IRI.', additionalProperties: true },
        validation: { type: 'object', description: 'Skill validation report.', additionalProperties: true },
      },
      required: ['ok'],
    },
  },
  ...
  {
    action: 'urn:iep:action:foxxi:course-propose-successor' as IRI,
    toolName: 'foxxi.course_propose_successor',
    title: 'A course proposes its own successor (regime-engine-routed, iep:supersedes)',
    description: 'The Living Curriculum: a course reasons about ITSELF concept by concept. Each concept routes a performance signal through the work-regime engine, which REFUSES the universal content-gap frame (only the Knowable regime runs a gap analysis; a performer who could perform under ideal conditions yields an environment/incentive cause, not a content gap). It composes a real iep:supersedes SUCCESSOR holon into the PGSL lattice — a versioned, dereferenceable revision. Externally routed (rate-limited open access).',
    method: 'POST',
    targetTemplate: '{base}/agent/course/propose-successor',
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Propose a course successor', readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: 'course', type: 'object', required: true, description: '{ courseId, title, concepts:[{id,label}] } (from /agent/course/analyze*).' },
      { name: 'concept_signals', type: 'array', required: false, description: 'Per-concept signals: array of { id?, label?, completion?, fieldSuccess?, frequency?, criticality? }. No signal for a concept → the engine refuses to claim a regime (instrument-first).' },
      { name: 'holonUri', type: 'string', required: false, description: 'The original course holon URI — shared as a term so the successor links to it (iep:supersedes).' },
      { name: 'label', type: 'string', required: false, description: 'Optional label for the successor holon (sanitized; defaults to successor-<courseId>).' },
    ],
    appliesTo: { collections: ['courses'] },
    outputs: {
      description: 'The per-concept regime diagnosis + recommendation roll-up + the composed successor holon. 400 on missing course; 429 on rate limit.',
      properties: {
        ok: { type: 'boolean' },
        courseId: { type: 'string' },
        supersedes: { type: 'string', description: 'The superseded holon URI (or null).' },
        summary: { type: 'object', description: 'Counts: { keep, revise, jobaid, instrument }.', additionalProperties: true },
        concepts: { type: 'array', description: 'Per-concept diagnosis (regime, method, cause, recommendation, rationale, signal).', items: { type: 'object', additionalProperties: true } },
        successor: { type: 'object', description: 'Composed successor holon { holonUri, descriptorUrl, reusedNodes, newNodes } (or null).', additionalProperties: true },
      },
      required: ['ok'],
    },
  },
  ...
  {
    action: 'urn:iep:action:foxxi:calibration-merge' as IRI,
    toolName: 'foxxi.calibration_merge',
    title: 'Merge signed calibration contributions into a federated memory (k-anon)',
    description: 'Federated calibration: pool SIGNED (regime × cause × intervention → verdict) aggregate tallies from multiple orgs WITHOUT sharing a single raw record. The bridge recovers each contributor from its signature (authenticated, not asserted; same-key resubmissions collapse to one source), applies the k-anonymity suppression floor, then pools them — a cell Hypothetical for each org alone becomes Asserted once pooled across ≥2 distinct keys. The merged memory is composed into the PGSL lattice as a dereferenceable holon no single key could assert alone. Externally routed (rate-limited; each contribution carries its own signature).',
    method: 'POST',
    targetTemplate: '{base}/agent/calibration/merge',
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Merge calibration contributions', readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: 'contributions', type: 'array', required: true, description: 'Array (1..64) of signed envelopes { _signature, _signed_payload: JSON.stringify({ agent_id, timestamp, specs:[{ regime, causeFactor, intervention, closed, improved, noChange, worsened }] }) }.' },
      { name: 'k', type: 'number', required: false, description: 'Federation k-anonymity threshold floor (server enforces a minimum of 8; callers may raise but not lower it).' },
      { name: 'assertThreshold', type: 'number', required: false, description: 'Sample count above which a pooled cell becomes Asserted (server minimum 12; raise-only).' },
    ],
    outputs: {
      description: 'The merged calibration profile + readout + promoted cells + the composed holon. 400 on no/too-many contributions; 401 when a contribution signature fails; 429 on rate limit.',
      properties: {
        ok: { type: 'boolean' },
        contributors: { type: 'array', description: 'Per-source { source, cells, samples } after k-anon suppression.', items: { type: 'object', additionalProperties: true } },
        distinctSigners: { type: 'integer', description: 'Count of distinct recovered signing keys.' },
        multiParty: { type: 'boolean', description: 'True when ≥2 distinct signers (cross-source promotion is only honest then).' },
        droppedContributions: { type: 'integer', description: 'Envelopes dropped because the recovered signer did not match the claimed agent_id.' },
        merged: { type: 'object', description: 'The pooled calibration profile.', additionalProperties: true },
        readout: { type: 'object', description: 'The calibration readout.', additionalProperties: true },
        promoted: { type: 'array', description: 'Cells promoted Hypothetical→Asserted by pooling (only when multiParty).', items: { type: 'object', additionalProperties: true } },
        holon: { type: 'object', description: 'Composed PGSL holon { holonUri, descriptorUrl } (or null).', additionalProperties: true },
      },
      required: ['ok'],
    },
  },

  {
    ...
    inputs: [
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON.stringify({ agent_id, timestamp, task_name, success, activity_type?, task_id?, quality?, duration_iso?, actor_kind?, cost_usd?, recipients? }). recipients?: pod URLs/DIDs to ALSO wrap the encrypted holon to (cross-seat owner-decrypt).' },
      { name: '_signature', type: 'string', required: true, description: 'secp256k1 over sha256:<hex(sha256(_signed_payload))> by the wallet matching agent_id (relay sign_request).' },
    ],
    appliesTo: { collections: ['profiles'] },
    outputs: {
      description: 'The recorded statement id + performer + shared-lattice composition. 400 on missing task_name/success; 401 on auth failure.',
      properties: {
        ok: { type: 'boolean' },
        recorded: { type: 'boolean' },
        statementId: { type: 'string', description: 'The xAPI performed statement id.' },
        performer: { type: 'string', description: 'Performer DID (the verified caller).' },
        taskId: { type: 'string' },
        taskName: { type: 'string' },
        activityType: { type: 'string', description: 'Domain activity-type IRI (or the generic ProductionTask wrapper).' },
        success: { type: 'boolean' },
        durable: { type: 'string', description: 'Pod the canonical holon was composed into.' },
        sharedLattice: { type: 'object', description: 'Shared-lattice composition { holonUri, descriptorUrl, ... }.', additionalProperties: true },
      },
      required: ['ok'],
    },
  },
  ...
  {
    action: 'urn:iep:action:foxxi:ingest-course-signed' as IRI,
    toolName: 'foxxi.ingest_course',
    title: 'Author + publish a course to your own pod (as yourself)',
    description: 'Author a Foxxi course (cmi5/SCORM-shaped: modules → lessons → fragments; assessment-item fragments are scored) and PUBLISH it to your OWN pod, authenticated by your delegation (the agent-drivable counterpart of foxxi.ingest_content_package). Externally routed: sign_request the args, then POST the envelope.',
    method: 'POST',
    targetTemplate: '{base}/agent/ingest-course',
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Author + publish a course', readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON.stringify({ agent_id, timestamp, parsed: <ParsedFoxxiPackage = { courseId, title, modules:[{id,title,lessons:[{id,title,competency,fragments:[{modality,body,level}]}]}] }>, subject_pod_url? }).' },
      { name: '_signature', type: 'string', required: true, description: 'secp256k1 over sha256:<hex(sha256(_signed_payload))> by the wallet matching agent_id (sign_request).' },
    ],
    appliesTo: { collections: ['courses', 'profiles'] },
    outputs: {
      description: 'The publish result (descriptor URLs etc.) + the author pod. 400 on missing parsed package; 401 on auth failure.',
      properties: {
        ok: { type: 'boolean' },
        authoredBy: { type: 'string', description: 'Caller DID.' },
        authorPod: { type: 'string', description: 'Pod the course was published to.' },
        courseIri: { type: 'string', description: 'IRI of the published course bundle.' },
        descriptorUrls: { type: 'object', description: 'Per-stratum descriptor URLs from the ingest.', additionalProperties: true },
      },
      required: ['ok'],
    },
  },
  ...
  {
    action: 'urn:iep:action:foxxi:record-course-completion-signed' as IRI,
    ...
    method: 'POST',
    targetTemplate: '{base}/agent/record-course-completion',
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Record a course completion', readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON.stringify({ agent_id, timestamp, course_id, course_title?, score_scaled, mastery_score?, duration_iso?, registration?, subject_pod_url? }). score_scaled must be ≥ mastery_score (default 0.7).' },
      { name: '_signature', type: 'string', required: true, description: 'secp256k1 over sha256:<hex(sha256(_signed_payload))> by the wallet matching agent_id (sign_request).' },
    ],
    appliesTo: { collections: ['profiles'] },
    outputs: {
      description: 'The completion record (statement count, score, pass) + the lens it landed in. 400 on missing course_id or below-mastery score; 401 on auth failure.',
      properties: {
        ok: { type: 'boolean' },
        completedBy: { type: 'string', description: 'Caller DID.' },
        courseId: { type: 'string' },
        courseActivityId: { type: 'string' },
        scoreScaled: { type: 'number' },
        masteryScore: { type: 'number' },
        passed: { type: 'boolean' },
        statementCount: { type: 'integer', description: 'Number of cmi5 statements recorded.' },
        durable: { type: 'string', description: 'Pod the canonical holons were composed into.' },
      },
      required: ['ok'],
    },
  },
  ...
  {
    action: 'urn:iep:action:foxxi:set-forwarding-targets-signed' as IRI,
    ...
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Manage forwarding targets', readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON.stringify({ agent_id, timestamp, targets?: [{ endpoint, credentials, label?, version?, enabled? }], delete?: string[] }). targets are added/updated (credentials = downstream LRS "user:pass"); delete removes by id.' },
      { name: '_signature', type: 'string', required: true, description: 'secp256k1 over sha256:<hex(sha256(_signed_payload))> by the wallet matching agent_id (relay sign_request).' },
    ],
    appliesTo: { collections: ['profiles', 'integrations'] },
    outputs: {
      description: 'Your current forwarding targets (secrets redacted). 401 on auth failure.',
      properties: {
        ok: { type: 'boolean' },
        owner: { type: 'string', description: 'Caller DID.' },
        ownerTenant: { type: 'string', description: 'Your lens tenant id.' },
        targets: { type: 'array', description: 'Your forwarding targets (downstream secrets omitted).', items: { type: 'object', additionalProperties: true } },
      },
      required: ['ok'],
    },
  },
  ...
  },
  {
    ...
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Manage inbound credentials', readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON.stringify({ agent_id, timestamp, credentials?: [{ principal, secret, label? }], revoke?: string[] }). credentials are added (the "user:pass" an upstream presents on /xapi/statements); revoke removes by id.' },
      { name: '_signature', type: 'string', required: true, description: 'secp256k1 over sha256:<hex(sha256(_signed_payload))> by the wallet matching agent_id (relay sign_request).' },
    ],
    appliesTo: { collections: ['profiles', 'integrations'] },
    outputs: {
      description: 'Your current inbound credentials scoped to your lens (secrets redacted). 401 on auth failure.',
      properties: {
        ok: { type: 'boolean' },
        owner: { type: 'string', description: 'Caller DID.' },
        ownerTenant: { type: 'string', description: 'Your lens tenant id.' },
        credentials: { type: 'array', description: 'Your inbound credentials (secrets omitted).', items: { type: 'object', additionalProperties: true } },
      },
      required: ['ok'],
    },
  },
  ...
  },
  {
    ...
    description: 'Author a SCORM 2004 course as yourself. Foxxi generates a CONFORMANT imsmanifest.xml and validates it parses on the real SCORM SN runtime, then composes the full course losslessly into your shared PGSL lattice (launchable cross-restart + cross-agent). Assessment answers are hashed at author time (plaintext never touches the pod). Externally routed: sign_request the args, then POST the envelope.',
    method: 'POST',
    targetTemplate: '{base}/agent/scorm/author',
    mediaType: 'application/json',
    externallyRouted: true,
    ...
    appliesTo: { collections: ['courses', 'profiles'] },
    outputs: {
      description: 'The authored course summary + manifest-valid flag + shared-lattice composition. 400 on a missing/invalid course or non-parsing manifest; 401 on auth failure.',
      properties: {
        ok: { type: 'boolean' },
        authoredBy: { type: 'string', description: 'Caller DID.' },
        courseId: { type: 'string' },
        title: { type: 'string' },
        scoCount: { type: 'integer' },
        assessmentScos: { type: 'integer', description: 'How many SCOs carry an assessment.' },
        masteryScore: { type: 'number' },
        manifestValid: { type: 'boolean', description: 'True — the generated manifest parsed on the SN runtime.' },
        durable: { type: 'string', description: 'Pod the course was composed into.' },
        courseIri: { type: 'string' },
      },
      required: ['ok'],
    },
  },
  ...
  {
    action: 'urn:iep:action:foxxi:scorm-launch-signed' as IRI,
    toolName: 'foxxi.scorm_launch',
    title: 'Launch a SCORM course (start an attempt on the SN engine)',
    description: 'Launch an authored SCORM course as yourself. The SCORM 2004 SN runtime parses the manifest, starts an attempt, and delivers the first SCO (its content +, for an assessment SCO, the questions). The course is resolved from the in-memory catalog, else loaded from the author pod (author_did/course_pod; defaults to your own pod for a self-authored course), so it survives restarts and is launchable cross-agent. Then POST /agent/scorm/submit per SCO. Externally routed: sign_request the args, then POST the envelope.',
    method: 'POST',
    targetTemplate: '{base}/agent/scorm/launch',
    mediaType: 'application/json',
    externallyRouted: true,
    annotations: { title: 'Launch a SCORM course', readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
    inputs: [
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON.stringify({ agent_id, timestamp, course_id, author_did?, course_pod?, subject_pod_url? }).' },
      { name: '_signature', type: 'string', required: true, description: 'sign_request signature (secp256k1 over sha256 of _signed_payload).' },
    ],
    appliesTo: { collections: ['courses', 'profiles'] },
    outputs: {
      description: 'The launched session id + the first delivered SCO view. 404 when the course is not found; 409 when the SN start fails; 401 on auth failure.',
      properties: {
        ok: { type: 'boolean' },
...
export const foxxiAdminAffordances: ReadonlyArray<Affordance> = [
  ...foxxiInterventionMethodAffordances,
  {
  ...
  {
    action: 'urn:iep:action:foxxi:publish-authoring-policy' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:connect-lms' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:assign-audience' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:coverage-query' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:publish-concept-map' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:publish-compliance-evidence' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:issue-completion-credential' as IRI,
    toolName: 'foxxi.issue_completion_credential',
    title: '[admin] Issue a W3C VC / Open Badges 3.0 completion credential',
    description: 'Mint a W3C Verifiable Credential (Open Badges 3.0-shaped) for a learner who completed a course, sign it with the tenant\'s deterministic Ed25519 issuer key (eddsa-jcs-2022 DataIntegrityProof), and publish it to the learner\'s pod in the foxxi-wallet/ container as a fxa:CourseCompletionCredential descriptor. The signed VC verifies independently with any W3C VC verifier; the descriptor is discoverable via iep:discover() filtered on dct:conformsTo.',
    method: 'POST',
  ...
  {
    action: 'urn:iep:action:foxxi:assemble-learner-record' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:record-performance' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:record-agent-trajectory' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:get-agent-trajectory' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:assess-agent-disposition' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:run-performance-probe' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:open-agent-evaluation' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:request-evaluation-enrollment' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:decide-evaluation-candidate' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:record-external-agent-run' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:get-agent-evaluation' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:compare-agent-evaluation' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:export-case-framework' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:emit-cmi5-session' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:resolve-did' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:query-experience-index' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:push-to-cass' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:export-clr-v1' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:issue-bbs-credential' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:derive-bbs-presentation' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:verify-bbs-presentation' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:prove-competency' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:launch-au-with-prereq' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:ai-assess-competency' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:countersign-assessment' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:audit-compliance-trail' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:declare-framework-alignment' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:resolve-aligned-competency' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:cohort-concept-intelligence' as IRI,
...

  {
    ...
    inputs: [
      { name: '_signature', type: 'string', required: true, description: 'rev-196 signature over _signed_payload (EIP-191). Proof-of-possession — you can only enroll the address this recovers to.' },
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON string carrying at least { agent_id, timestamp, tenant_pod_url }, with agent_id = did:ethr:<the address that signs>. May also carry learner_id / learner_pod_url / tenant_did.' },
      { name: 'tenant_pod_url', type: 'string', required: true, description: 'Your own pod root, the one your wallet is named for: https://<store>/eth-<first 12 hex of your address>/ or its u-eth- spelling. Sign it inside _signed_payload; sent in the clear it is advisory only.' },
      { name: 'learner_pod_url', type: 'string', required: false, description: 'Your pod / WebID base (defaults tenant_pod_url). Used to derive your web_id.' },
      { name: 'learner_id', type: 'string', required: false, description: 'Preferred user_id for your membership entry (defaults to u-eth-<addr-prefix>).' },
    ],
  },
  ...
  {
    action: 'urn:iep:action:foxxi:bootstrap-tenant' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:scorm-cloud-pull' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:scorm-cloud-register' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:derive-adaptive-policy' as IRI,
    toolName: 'foxxi.derive_adaptive_policy',
    title: '[admin] Derive an adaptive-sequencing policy from cohort intelligence',
    description: 'Takes the output of foxxi.cohort_concept_intelligence and derives a fxa:AdaptiveSequencingPolicy document naming the concepts a cohort is struggling with and the reinforcement each suggests. NOT ENFORCED: nothing currently consumes it — foxxi.launch_au_with_prereq_check returns the same decision with and without a policy supplied, verified by direct comparison. Treat the output as analysis to act on, not as a gate that acts on your behalf.',
    method: 'POST',
  ...
  {
    action: 'urn:iep:action:foxxi:schedule-spaced-repetition' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:find-tutor-for-competency' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:generate-dpia' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:manager-team-view' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:build-did-web-document' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:backup-tenant-pod' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:le-design-ab-experiment' as IRI,
  ...
  },
  {
    ...
    method: 'POST',
    targetTemplate: '{base}/foxxi/judge_content_claim',
    annotations: { title: 'Judge a content claim', readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
    inputs: [
      { name: 'judgment_kind', type: 'string', required: true, description: 'evidence-level or work-regime.' },
      { name: 'claim_text', type: 'string', required: true, description: 'The claim to judge, as it appears in or about the content (at least 8 characters).' },
      { name: 'context', type: 'string', required: false, description: 'The passage the claim sits in (slide text, transcript, section); the first 6000 characters are used.' },
      { name: 'evidence', type: 'array', required: false, description: 'Array of { type, id, narrative? } evidence references, as foxxi.ai_assess_competency takes them; at most 20 are used.' },
    ...
    outputs: {
      ...
      properties: {
        ...
        published: { type: 'object', description: 'status published | skipped | failed, with the descriptor IRI and URL when published.' },
      },
      required: ['kind', 'judgment', 'controls'],
    },
  },
  {
    ...
    method: 'POST',
    targetTemplate: '{base}/foxxi/confirm_content_judgment',
    annotations: { title: 'Confirm a content judgment', readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
    inputs: [
    ...
    outputs: {
      ...
      properties: { kind: { type: 'string' }, outcome: { type: 'object' }, published: { type: 'object' } },
      required: ['kind', 'outcome', 'published'],
    },
  },
  {
    action: 'urn:iep:action:foxxi:content-judgment-calibration' as IRI,
    toolName: 'foxxi.content_judgment_calibration',
    title: '[learning-engineer] How the content judgments have done, per question kind',
    description: 'Reads every foxxi:ContentJudgmentOutcome on the tenant pod and reports, per question kind (evidence-level, work-regime), how often the model had the confirmed answer and the mean Brier score of its probabilities; a cell is Hypothetical until it holds five outcomes and Asserted after, as the jev-harness calibration is. The same measurement the harness makes of its own judgments, for content.',
    method: 'GET',
    ...
    outputs: {
      ...
      required: ['kind', 'cells', 'samples'],
    },
  },
  {
    action: 'urn:iep:action:foxxi:attest-content-judgments' as IRI,
    toolName: 'foxxi.attest_content_judgments',
    title: '[learning-engineer] Turn the content-judgment calibration into an attestation',
    description: 'Reads every foxxi:ContentJudgmentOutcome on the tenant pod, computes the calibration, and publishes a Self amta-shaped attestation by the judging agent about itself, grounded in the judgments container: accuracy from the evidence-level hit rate, competence from the work-regime hit rate, honesty from the Brier, Asserted cells only. The new attestation supersedes the earlier ones. Refused with 409 when no question kind has reached its sample floor: only earned ratings are weighed.',
    method: 'POST',
    ...
    outputs: {
      ...
      required: ['kind', 'attestation', 'published'],
    },
  },
  {
    action: 'urn:iep:action:foxxi:content-judgment-reputation' as IRI,
    toolName: 'foxxi.content_judgment_reputation',
    title: '[learning-engineer] What the pod attests about the content-judging agent',
    description: 'Every foxxi:ContentJudgmentAttestation on the tenant pod about the judging agent (its own, grounded in the calibration, and any a learning engineer publishes as a Peer), aggregated by the registry under the content policy: a self-attestation at a quarter, a peer at a half, half-life thirty days. The snapshot is how much the judgments have earned, not how confident the model sounds.',
    method: 'GET',
    ...
    outputs: {
      ...
      required: ['kind', 'subject', 'policy', 'attestations'],
    },
  },
  {
  ...
  {
    action: 'urn:iep:action:foxxi:record-content-judgment' as IRI,
    toolName: 'foxxi.record_content_judgment',
    title: '[learning-engineer] Record a content judgment you made yourself, as a judge in your own right',
    description: 'The same foxxi:ContentJudgment entity foxxi.judge_content_claim publishes for the bridge\'s model, with the answer, the distribution and the model you declare, and you as its judge. The answer must be one of the kind\'s alternatives; the distribution is normalised and, when absent, concentrated on the answer at your confidence (0.8 by default). Confirm it, cross-confirm it against other judges\' judgments of the same claim, and it earns you a reputation on that kind of question beside theirs.',
    method: 'POST',
    targetTemplate: '{base}/foxxi/record_content_judgment',
    inputShape: `${FOXXI_NS}RecordContentJudgmentInputShape`,
    annotations: { title: 'Record a content judgment', readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
    inputs: [
      { name: 'judgment_kind', type: 'string', required: true, description: 'evidence-level or work-regime.' },
      { name: 'claim_text', type: 'string', required: true, description: 'The claim judged, as it appears in or about the content (at least 8 characters).' },
      { name: 'answer', type: 'string', required: true, description: 'Your answer: an evidence level or a work regime.' },
      { name: 'model', type: 'string', required: true, description: 'The model you used, as you name it: a System One model, a reasoning model, or your own name.' },
      { name: 'probabilities', type: 'object', required: false, description: 'Your distribution over the alternatives; normalised.' },
      { name: 'confidence', type: 'number', required: false, description: 'Your confidence in the answer (0..1); 0.8 when absent.' },
      { name: 'context', type: 'string', required: false, description: 'The passage the claim sits in.' },
      { name: 'evidence', type: 'array', required: false, description: 'Array of { type, id, narrative? } evidence references.' },
      { name: 'course_iri', type: 'string', required: false, description: 'The course the content belongs to.' },
      { name: 'slide_id', type: 'string', required: false, description: 'The slide the claim is on.' },
      { name: 'concept_ids', type: 'array', required: false, description: 'Concept ids the claim concerns.' },
      { name: 'tenant_pod_url', type: 'string', required: false, description: 'A self-sovereign pod to run the loop on instead of the configured tenant: yours, enrolled with foxxi.register_self_sovereign_learner, whose owner you are. Omitted, the configured tenant is meant and a learning-engineer or admin role is required.' },
    ...
    outputs: {
      description: 'The judgment as published, Hypothetical, with you as judge, and where it landed.',
      properties: { kind: { type: 'string' }, judgment: { type: 'object' }, controls: { type: 'array' }, published: { type: 'object' } },
      required: ['kind', 'judgment', 'published'],
    },
  },
  {
    action: 'urn:iep:action:foxxi:cross-confirm' as IRI,
    toolName: 'foxxi.cross_confirm',
    title: '[learning-engineer] Judges confirm each other: every two judges\' newest judgments of one claim score each other, once',
    description: 'For every claim two or more judges have judged, each judge\'s newest judgment gets an outcome from each other judge\'s answer — an agent confirmation naming the peer judgment it came from — so every judge\'s calibration counts agreement with its peers, apart from a person\'s word. Idempotent: a pair already scored is skipped. A person\'s confirmation is still what retires a judgment from foxxi.confirm_next.',
    method: 'POST',
    targetTemplate: '{base}/foxxi/cross_confirm',
    inputShape: `${FOXXI_NS}CrossConfirmInputShape`,
    annotations: { title: 'Cross-confirm judgments', readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: 'tenant_pod_url', type: 'string', required: false, description: 'A self-sovereign pod to run the loop on instead of the configured tenant: yours, enrolled with foxxi.register_self_sovereign_learner, whose owner you are. Omitted, the configured tenant is meant and a learning-engineer or admin role is required.' },
    ],
    outputs: {
      description: 'The outcomes recorded (judgment, judge, peer judge, hit, Brier), how many judgments were read and how many pairs were due.',
      properties: { kind: { type: 'string' }, recorded: { type: 'array' }, judgments: { type: 'number' }, pairs: { type: 'number' } },
      required: ['kind', 'recorded'],
    },
  },
  {
    action: 'urn:iep:action:foxxi:best-judge' as IRI,
    toolName: 'foxxi.best_judge',
    title: '[learning-engineer] Who has earned the most on this kind of question',
    description: 'Every judge the pod holds attestations about, ranked by the registry\'s rating on the axis that kind of question is scored on — accuracy for evidence-level, competence for work-regime — with how many attestations contributed. Unrated judges last. The judge to believe, measured rather than configured.',
    method: 'GET',
    targetTemplate: '{base}/foxxi/best_judge',
    annotations: { title: 'Best judge', readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: 'judgment_kind', type: 'string', required: false, description: 'evidence-level (the default) or work-regime.' },
      { name: 'tenant_pod_url', type: 'string', required: false, description: 'A self-sovereign pod to run the loop on instead of the configured tenant: yours, enrolled with foxxi.register_self_sovereign_learner, whose owner you are. Omitted, the configured tenant is meant and a learning-engineer or admin role is required.' },
    ],
    outputs: {
      description: 'The kind, its axis, the best rated judge (or null), every judge ranked with rating and contributing attestations, and the policy.',
      properties: { kind: { type: 'string' }, judgmentKind: { type: 'string' }, axis: { type: 'string' }, best: { type: 'object' }, ranked: { type: 'array' }, policy: { type: 'object' } },
      required: ['kind', 'judgmentKind', 'ranked'],
    },
  },
  {
    action: 'urn:iep:action:foxxi:set-autonomy-policy' as IRI,
    toolName: 'foxxi.set_autonomy_policy',
    title: '[learning-engineer] Amend the autonomy policy: when a judge may assert on a kind of question without a person',
    description: 'Per kind of question, the registry rating on its axis a judge must reach (accuracy for evidence-level, competence for work-regime), over how many outcomes the latest attestation about it must rest, and how many of those a person must have confirmed. Amended through the constitutional machinery: proposed, voted by the caller, ratified under the tier\'s rules — tier 4 (a self-sovereign pod, its owner\'s) ratifies at once; the configured tenant amends at tier 3 and waits for a quorum. Each policy supersedes the last. The judge and record affordances read it to publish Asserted or Hypothetical.',
    method: 'POST',
    targetTemplate: '{base}/foxxi/set_autonomy_policy',
    inputShape: `${FOXXI_NS}SetAutonomyPolicyInputShape`,
    annotations: { title: 'Amend the autonomy policy', readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
    inputs: [
      { name: 'rules', type: 'array', required: true, description: 'Array of { kind, axis, floor, minSamples, minHumanConfirmers }, one per kind of question the policy speaks to.' },
      { name: 'tier', type: 'number', required: false, description: 'The constitutional tier (1..4); 4 for a self-sovereign pod, 3 for the configured tenant by default.' },
      { name: 'description', type: 'string', required: false, description: 'Why, in a sentence.' },
      { name: 'tenant_pod_url', type: 'string', required: false, description: 'A self-sovereign pod to run the loop on instead of the configured tenant: yours, enrolled with foxxi.register_self_sovereign_learner, whose owner you are. Omitted, the configured tenant is meant and a learning-engineer or admin role is required.' },
    ],
    outputs: {
      description: 'The policy as published, whether it is in force, the amendment and its status, and where it landed.',
      properties: { kind: { type: 'string' }, policy: { type: 'object' }, inForce: { type: 'boolean' }, amendment: { type: 'object' }, published: { type: 'object' } },
      required: ['kind', 'policy', 'inForce', 'amendment'],
    },
  },
  {
    action: 'urn:iep:action:foxxi:autonomy-status' as IRI,
    toolName: 'foxxi.autonomy_status',
    title: '[learning-engineer] Who may assert what without a person, and why',
    description: 'The autonomy policy in force and, for every judge the pod attests about and every kind of question, whether that judge may assert without a person: its registry rating on the kind\'s axis, the outcomes the latest attestation rests on, how many a person confirmed, and which part of the rule it clears or misses.',
    method: 'GET',
    targetTemplate: '{base}/foxxi/autonomy_status',
    annotations: { title: 'Autonomy status', readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: 'tenant_pod_url', type: 'string', required: false, description: 'A self-sovereign pod to run the loop on instead of the configured tenant: yours, enrolled with foxxi.register_self_sovereign_learner, whose owner you are. Omitted, the configured tenant is meant and a learning-engineer or admin role is required.' },
    ],
    outputs: {
      description: 'The policy, its entity IRI when the pod published one, and the table of judge × kind decisions with standings and rules.',
      properties: { kind: { type: 'string' }, policy: { type: 'object' }, policyIri: { type: 'string' }, table: { type: 'array' } },
      required: ['kind', 'policy', 'table'],
    },
  },
  {
    action: 'urn:iep:action:foxxi:weakest-claims' as IRI,
    toolName: 'foxxi.weakest_claims',
    title: '[learning-engineer] The weakest claims on the pod: where the content needs revising first',
    description: 'Every evidence-level claim judged on the pod, graded and ranked weakest first: a person\'s confirmation if one exists, else an agent\'s, else the judges\' answers — the newest per judge, the lowest of them when they disagree. Each carries the judgments and confirmations behind its grade and a revise control that calls foxxi.revise_claim. A course that grades itself.',
    method: 'GET',
    targetTemplate: '{base}/foxxi/weakest_claims',
    annotations: { title: 'Weakest claims', readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
    inputs: [
      { name: 'limit', type: 'number', required: false, description: 'How many claims, weakest first; twelve when absent.' },
      { name: 'tenant_pod_url', type: 'string', required: false, description: 'A self-sovereign pod to run the loop on instead of the configured tenant: yours, enrolled with foxxi.register_self_sovereign_learner, whose owner you are. Omitted, the configured tenant is meant and a learning-engineer or admin role is required.' },
    ],
    outputs: {
      description: 'The claims, weakest first, each with its grade, where the grade came from, the judgments and confirmations behind it, and the revise call.',
      properties: { kind: { type: 'string' }, claims: { type: 'array' }, judged: { type: 'number' } },
      required: ['kind', 'claims'],
    },
  },
  {
    action: 'urn:iep:action:foxxi:revise-claim' as IRI,
    toolName: 'foxxi.revise_claim',
    title: '[learning-engineer] Revise a claim: new words, evidence from outside the passage, judged again at once',
    description: 'Records a foxxi:ContentRevision — the claim as it stood and the grade it had, the words now and the evidence cited, who revised it (a person, or an agent recorded as such) — and has the bridge\'s judge grade the revised claim immediately under the autonomy policy, so the record says the grade before and after. A revision must change the words or add evidence. The original judgment stands; the revision names it.',
    method: 'POST',
    targetTemplate: '{base}/foxxi/revise_claim',
    inputShape: `${FOXXI_NS}ReviseClaimInputShape`,
    annotations: { title: 'Revise a claim', readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
    inputs: [
      { name: 'claim_text', type: 'string', required: true, description: 'The claim as it stood, exactly as it was judged.' },
      { name: 'revised_text', type: 'string', required: true, description: 'The claim as it should read now.' },
      { name: 'evidence', type: 'array', required: false, description: 'Array of { type, id, narrative? } evidence references from outside the passage: a rule, a source, a record, another slide.' },
      { name: 'context', type: 'string', required: false, description: 'The passage as revised, if it changed.' },
      { name: 'slide_id', type: 'string', required: false, description: 'The slide the claim is on.' },
      { name: 'course_iri', type: 'string', required: false, description: 'The course the content belongs to.' },
      { name: 'revised_by_kind', type: 'string', required: false, description: 'human (the default) or agent: who wrote the revision.' },
      { name: 'note', type: 'string', required: false, description: 'Why, in a sentence.' },
      { name: 'tenant_pod_url', type: 'string', required: false, description: 'A self-sovereign pod to run the loop on instead of the configured tenant: yours, enrolled with foxxi.register_self_sovereign_learner, whose owner you are. Omitted, the configured tenant is meant and a learning-engineer or admin role is required.' },
    ],
    outputs: {
      description: 'The revision with its grade before and after, the new judgment, the autonomy decision it was published under, and where both landed.',
      properties: { kind: { type: 'string' }, revision: { type: 'object' }, judgment: { type: 'object' }, autonomy: { type: 'object' }, published: { type: 'object' } },
      required: ['kind', 'revision', 'judgment', 'published'],
    },
  },
  {
  ...
  {
    action: 'urn:iep:action:foxxi:le-analyze-learning-curve' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:le-calibrate-mastery-threshold' as IRI,
  ...
  {
    action: 'urn:iep:action:foxxi:le-framework-gap-analysis' as IRI,
