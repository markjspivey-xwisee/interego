...
export const foxxiAffordances: ReadonlyArray<Affordance> = [
...

  {
    ...
    description: 'Holder-facing BBS+ proof: the issuer (credentialing authority) signs a multi-claim credential; the holder discloses only a minimal privacy-preserving subset (issuer + achievement name + proficiency by default) and cryptographically HIDES the rest (score, dates, name, id); a verifier learns ONLY the disclosed claims. Real W3C bbs-2023 crypto. Externally routed: sign_request the args as the HOLDER, then POST the envelope.',
    method: 'POST',
    ...
    inputs: [
      { name: '_signed_payload', type: 'string', required: true, description: 'JSON.stringify({ agent_id, timestamp, issuer_did, competency_name, score?, proficiency?, reveal? }). reveal is an array of claim paths to disclose (default ["issuer","achievement.name","achievement.proficiencyLevel"]).' },
      { name: '_signature', type: 'string', required: true, description: 'sign_request signature by the holder (subject of the credential).' },
    ],
    appliesTo: { collections: ['profiles'] },
    outputs: {
      description: 'The serialized BBS+ selective-disclosure presentation + which claims were revealed vs cryptographically hidden. 503 when FOXXI_ISSUER_KEY_SEED is unset.',
      properties: {
        ok: { type: 'boolean' },
        holder: { type: 'string', description: 'Holder DID.' },
        issuerDid: { type: 'string', description: 'Issuer DID that signed the credential.' },
        credentialId: { type: 'string', description: 'Underlying credential id.' },
        totalClaims: { type: 'integer', description: 'Number of signed claims.' },
        ...
        presentation: { type: 'object', description: 'Serialized BBS+ presentation (base64 proof + disclosed messages + issuer public key) for the verifier.', additionalProperties: true },
      },
      required: ['ok'],
    },
  },
