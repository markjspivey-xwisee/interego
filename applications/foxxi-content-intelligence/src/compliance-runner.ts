...
export async function runXapiConformance(opts: XapiOpts): Promise<ComplianceReport> {
  ...
  return tally('xapi-2.0', 'xAPI 2.0 LRS Conformance', 'IEEE 9274.1.1 (xAPI 2.0) + xAPI Profile Spec 2017 — live battery across §3/§4/§6/§7 + the §4 validator (whose vocab/patterns are single-sourced from the dereferenceable /ns/xapi ontology) on every externally-submitted statement (internally-emitted statements are built from typed, ontology-aligned builders), plus direct SHACL validation against the composed /ns/xapi shapes. (The complete external ADL lrs-conformance-test-suite (LRS-2.0) result is attested out-of-band — it is NOT reproduced by this endpoint, which runs the live battery above.)', B, ranAt, checks);
}
