...
export function makePrivatePerformanceVerifier(config: {
  ...
  /** Rewrite only configured aliases of this deployment's store; preserve foreign origins. */
  canonicalPod: (pod: string) => string;
  ...
  const samePod = (a: string | undefined, b: string): boolean =>
    a !== undefined && config.samePod(config.canonicalPod(a), config.canonicalPod(b));
  return async (body, write) => {
    ...
    if (!samePod(config.ownPod(auth.callerDid), pod)) return { ok: false, status: 403, error: 'Verified actor does not own the selected account.' };
    const stamped = auth.payload.subject_pod_url;
    if (stamped !== undefined && (typeof stamped !== 'string' || !samePod(stamped, pod))) return { ok: false, status: 403, error: 'Signed pod stamp does not match the verified own pod; steering refused.' };
    ...
    if (!samePod(vc?.pod, pod) || !scope.includes('discover') || (write && !scope.includes('publish'))) return { ok: false, status: 403, error: 'Private performance requires own-pod discover scope and publish scope for writes (ordinary ReadWrite delegation).' };
    return auth;
  };
}
