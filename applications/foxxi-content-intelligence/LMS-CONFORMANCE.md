...
| AU auth-token accepted by the LRS, scoped to its tenant **and its launch**: the statements it writes must carry its registration, a query is held to it, a statement outside it is not found, and the State it reads and writes is its own launch's | **Implemented** | `cmi5BearerTenant` + `cmi5BearerRegistration` → LRS auth gate; `tests/cmi5-token-is-its-launch.test.ts` |
