...
| **Context Companion** — one conversational front door over a user's networked context: `POST /content/ask` classifies intent and answers from the substrate's own surfaces — assignments, the live LRS, and (for content) the vertical's existing **agentic RAG** (LLM-synthesised or retrieval-scaffold) — with sourced answers and an honest no-match, the same for humans and agents. `scope: interego` (default) passes through to the wider substrate via `discover()`; `scope: vertical` narrows to the Foxxi slice | shipped — [`src/context-chat.ts`](src/context-chat.ts); process recorded in [`ASKING-YOUR-CONTEXT.md`](ASKING-YOUR-CONTEXT.md) |
