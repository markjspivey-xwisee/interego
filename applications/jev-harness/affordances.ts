...
export const jevHarnessAffordances: ReadonlyArray<Affordance> = [
  ...
  {
    action: actionIri('urn:iep:action:jev-harness:draft-attestation'),
    toolName: 'jev_harness.draft_attestation',
    title: 'Draft a peer attestation about this agent',
    description: 'An amta:Attestation for YOU (a person, or the session agent acting for one) to publish about the harness agent from your own key: direction Peer, your ratings where you give them and the calibration ratings where you do not, grounded in the outcome you decided on or in the calibration. The bridge returns the graph and the exact publish_context arguments; it never publishes it itself, because a second voice has to be another key. Once published, GET /jev-harness/reputation weighs it as PeerAttested.',
    method: 'POST',
    targetTemplate: '{base}/jev-harness/attestation/draft',
    inputShape: `${NS}DraftAttestationInputShape`,
    returns: `${NS}AttestationDraft`,
    mediaType: 'application/json',
    annotations: { title: 'Draft an attestation', readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    inputs: [
      { name: 'attestor', type: 'string', required: true, description: 'The DID or WebID that will publish the attestation: you, or the session agent acting for you.' },
      { name: 'about', type: 'string', required: false, description: 'The descriptor URL the attestation is grounded in: an outcome you decided on. The calibration descriptor when absent.' },
      { name: 'note', type: 'string', required: false, description: 'Why, in a sentence; becomes the prose of the attestation.' },
      { name: 'accuracy', type: 'number', required: false, description: 'Your rating 0..1 of how the review verdict agrees with people; the calibration rating when absent.', minimum: 0, maximum: 1 },
      { name: 'competence', type: 'number', required: false, description: 'Your rating 0..1 of navigation; the calibration rating (hit@3) when absent.', minimum: 0, maximum: 1 },
      { name: 'relevance', type: 'number', required: false, description: 'Your rating 0..1 of test selection; the calibration rating when absent.', minimum: 0, maximum: 1 },
      { name: 'honesty', type: 'number', required: false, description: 'Your rating 0..1 of how faithfully probabilities report what happens; the calibration rating when absent.', minimum: 0, maximum: 1 },
    ],
    outputs: {
      description: 'The attestation graph, the publish_context call that publishes it, and where to read the reputation afterwards.',
      properties: {
        graphIri: { type: 'string', description: 'urn:graph:jev-harness:attestation:<repo>:peer:<attestor>; a newer word from the same attestor supersedes the older.' },
        content: { type: 'string', description: 'The amta:Attestation payload as Turtle.' },
        publish: { type: 'object', description: 'tool and arguments: call publish_context with these from your own session.' },
        reputationUrl: { type: 'string', description: 'GET it after publishing to see the snapshot weigh your word.' },
      },
      required: ['graphIri', 'content', 'publish', 'reputationUrl'],
    },
  },
];
