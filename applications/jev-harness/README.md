...
A pull request merges on the gate's verdict alone only when five things hold, each printed by the `gated auto-merge` job: the operator armed it (repository variable `JEV_AUTO_MERGE=true`); a token that triggers auto-deploy is present (secret `JEV_MERGE_TOKEN`, a fine-grained token with contents and pull-requests write — a merge made with the run's own `GITHUB_TOKEN` starts no workflows and would ship nothing); the selection ran green; the verdict is auto-ok; and the calibration the deployed bridge publishes shows at least 20 live review-verdict outcomes with agreement of 0.9 or more and disagreement of 0.05 or less. Replayed history does not count. Unarmed, the job prints the decision it would have taken and exits green, so the record accrues before anyone trusts it. `src/auto-merge.ts` is the policy; `bin/auto-merge.ts` runs it.
