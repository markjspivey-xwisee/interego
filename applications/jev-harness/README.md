...
- **Calibrated advice.** Static confidence bands offered open-top-file eleven times with no hits. The service now derives the advice from the measured hit rates of the judgment's confidence bucket once that bucket holds five outcomes, and says so on the judgment (`jvh:adviceBasis`). Until then the static bands apply. The controls a navigation affords therefore follow the evidence, which is the point of publishing outcomes at all.
