...
Give the bridge an agent key (`INTEREGO_AGENT_KEY_JSON`, an Ed25519 JWK): it is then its own relay agent with a did:key, mints its own relay tokens through the relay's OAuth flow and renews them before they lapse. When the judgments belong on a pod that is not the agent's own, name it (`INTEREGO_POD_NAME`); the pod owner registers the agent there with a publishing scope, and every descriptor is attributed to the agent on behalf of the owner. A pasted session token (`INTEREGO_BEARER`, an hour long) still works, and `INTEREGO_RELAY_URL` defaults to `https://relay.interego.xwisee.com/mcp`. `JEV_HARNESS_CONTROL_BASE` names the bridge that executable controls point at when it is not the publishing bridge (CI names the deployed one), and `JEV_HARNESS_REPO_NAME` names the repository when its directory does not (the Railway image runs at `/app`), so a deployed bridge and CI publish one calibration chain, `urn:graph:jev-harness:calibration:<name>`. Since 2026-09-21 the harness and the fleet's own evidence share one pod, the one the operator's live connector owns (`u-pk-f2a9c751075a`); the earlier history on `u-pk-33d42e6b33dc` stays readable there. Every judgment is published through the relay's `publish_context` with its payload graph, modal status and confidence, and the bridge records a `record_trajectory_step` for its own loop. The relay is the authority for facets, signing, encryption and supersession; the local `.trig` is for offline use. The MCP transport is exercised against a fake relay in [`tests/publish.test.ts`](tests/publish.test.ts); the live relay was exercised from a Claude session with the same `publish_context` arguments (see the dogfood log below).
...
`.github/workflows/jev-harness.yml` runs on every pull request: one job starts the bridge bound to the checkout and follows `select-tests` into the run, the triage when the run failed, and the outcome; the other gates the diff and fails only when the verdict is block, a secret in the diff; needs-human-review is advisory unless the repository variable `JEV_REQUIRE_HUMAN_REVIEW` is `true`, in which case it fails the job too. Each job leaves one comment on the pull request, edited in place on later runs: the verdict with the hazards that fired and links to the descriptor on the pod, or the selection with why it was chosen and the triage table. With `INTEREGO_AGENT_KEY_JSON` as a repository secret the runner's bridge publishes as the same delegate agent as the deployed one, so CI's judgments and outcomes land on the pod beside everyone else's. `TYPESAFE_API_KEY` is the other secret. When the pull request closes, a third job scores the judgments the comments name: the merge, or the close without one, is the human decision the review verdict is scored against, and a navigation a session linked to the pull request is scored against the files it changed. That bridge reads each judgment back from the pod (`src/pod-judgment.ts`), since the bridge that made it is gone with its run, and publishes the outcome as the Asserted head of the judgment's chain.
...
## Calibration on the pod, and what the harness says about itself

The calibration view is published to the pod as an Asserted `jvh:Calibration` under one graph IRI per repository (`urn:graph:jev-harness:calibration:<repo>`), each publish superseding the last, so the chain is the calibration history: per kind a `jvh:cell` with sample counts (live ones apart), status, hit rates, Brier and agreement counts; the `jvh:memory` split; the advice buckets. Once a cell has reached its sample floor the bridge also issues an `amta:Attestation` about itself — direction Self, a claim rather than a second opinion — with competence from navigation hit@3, accuracy from review-verdict agreement, relevance from test-selection coverage and honesty from one minus the navigation Brier, grounded in the calibration descriptor by `amta:fromExecution`. The bridge publishes after a pod read-back that added outcomes, CI after scoring a merged pull request, and `POST /jev-harness/calibration/publish` (the `publish-calibration` affordance) on demand; `/health` reports the last publish.

## Reputation from attestations

`GET /jev-harness/reputation` (the `reputation` affordance) reads every `amta:Attestation` about the harness agent on its pod — its own grounded self-attestation and any a peer publishes under `urn:graph:jev-harness:attestation:` — and aggregates them with `@interego/registry` under the harness policy: a grounded self-attestation counts at a quarter of a peer's word and half of a high-assurance one's (the registry's default gives it nothing, and rightly so for a bare claim; ours name the calibration descriptor they were computed from), with a thirty-day recency half-life. The answer carries the attestations read, the snapshot with its per-axis ratings and contributing descriptors, and the policy. With a person required, the gated auto-merge reads the snapshot's accuracy axis as its last condition, so the published evidence is consumed rather than recomputed.

### A peer's word

`POST /jev-harness/attestation/draft` (the `draft_attestation` affordance) drafts an `amta:Attestation` for someone else to publish: direction Peer, the attestor named as `attestor` (a person's WebID, or the session agent acting for them), their ratings where given and the calibration's where not, grounded in `about` (an outcome they decided on) or in the calibration descriptor. The answer carries the graph, the exact `publish_context` arguments to call from their own session, and the reputation URL to read afterwards. The bridge never publishes it: a second voice has to be another key. The graph IRI is `urn:graph:jev-harness:attestation:<repo>:peer:<attestor>`, so a newer word from the same attestor supersedes the older, and the policy weighs it as PeerAttested (0.5) beside the self-attestation (0.25).

## Gated auto-merge

A pull request merges without a person when seven things hold, each printed by the `gated auto-merge` job: the operator armed it (repository variable `JEV_AUTO_MERGE=true`); a token that triggers auto-deploy is present (secret `JEV_MERGE_TOKEN`, a fine-grained token with contents and pull-requests write — a merge made with the run's own `GITHUB_TOKEN` starts no workflows and would ship nothing); the selection ran green; the gate found no secret (a `block` verdict never merges; needs-human-review is advisory); GitHub reports the branch mergeable; master has not moved since the run's merge ref (a moved base is updated instead, and the next run decides); and the whole root suite that `bridge-typecheck.yml` runs on the same head passed — the job waits for that run before deciding (`tools/wait-for-workflow-run.mjs`), because on 2026-09-22 the suite was red on every pull request for fourteen hours and seven merged over it with nothing reading the result. `JEV_MERGE_WAITS_FOR_SUITE=false` makes the suite's word advisory, printed but not gating. The judge's own step is red when the selected run is (`bin/follow.ts` exits 1 on a failed run), and a selection in full mode — a sensitive path changed — is not run there at all: the whole suite runs in `bridge-typecheck.yml` on the same head, and the decision waits for it. With `JEV_REQUIRE_HUMAN_REVIEW=true` the bar rises: the verdict must be auto-ok and the calibration the deployed bridge publishes must show at least 20 live review-verdict outcomes with agreement of 0.9 or more and disagreement of 0.05 or less. An automatic merge is labelled `jev-harness:auto-merged` before the merge, and the close-time job then records no human decision for its verdict: a machine's merge is not a person's approval, and calibration only counts the latter. Unarmed, the job prints the decision it would have taken and exits green. `src/auto-merge.ts` is the policy; `bin/auto-merge.ts` runs it.
