 ...
 *     --selection-result success|failure|cancelled|skipped [--dry-run]
 *
 ...
 * passes; prints every condition and the decision; merges with `gh pr merge --merge` only
 * when every condition holds and --dry-run is absent (GH_TOKEN must then be the merge token,
 * not the run's own). Exits 0 unless the merge command itself fails: an unarmed or unearned
 * decision is a green job that says so, not a red one. src/auto-merge.ts is the policy.
 */

import { execFileSync } from 'node:child_process';
import { autoMergeDecision, type AutoMergeInputs } from '../src/auto-merge.js';
...
async function main(): Promise<void> {
  ...
  if (!pr || !repo || !dir || !calibrationUrl) {
    console.error('usage: --pr <n> --repo <owner/name> --dir <results dir> --calibration-url <url> --armed true|false --token-present true|false --selection-result <result> [--dry-run]');
    process.exit(2);
  }
  ...
  if (process.argv.includes('--dry-run')) { console.log('  dry run: the merge command was not run'); return; }
