 ...
 *
 *   tsx tools/backtest.ts --repo <path> [--commits 40] [--kinds navigate,select,gate] [--max-files 25] [--concurrency 3]
 *
...
interface Args { repo: string; commits: number; kinds: Set<string>; maxFiles: number; concurrency: number }

function parseArgs(argv: string[]): Args {
  const a: Args = { repo: process.cwd(), commits: 40, kinds: new Set(['navigate', 'select', 'gate']), maxFiles: 25, concurrency: 3 };
...
interface CaseResult {
  ...
  navigate?: { top: string; confidence: number; advice: string; hitAt1: boolean | null; hitAt3: boolean | null; brier: number | null; passes: number; tokens: number };
...
async function main(): Promise<void> {
  ...
  const cases = loadCases(args.repo, args.commits, args.maxFiles);
  ...
  console.error(`backtest: ${cases.length} commits, kinds ${[...args.kinds].join('+')}, worktree ${worktree}`);
  ...
  for (const [i, c] of cases.entries()) {
    ...
    const parts = [
      r.navigate ? `nav hit@1=${r.navigate.hitAt1} conf=${r.navigate.confidence}` : '',
  ...
  const lines = [
    ...
    '',
    `Repository ${args.repo}, ${results.length} commits (≤ ${args.maxFiles} files each), ${tokens} input tokens ≈ $${cost}.`,
    '',
    '| Kind | Metric | Value |',
    '| --- | --- | --- |',
