...
| **Real cross-bridge p2p**: Mark publishes a descriptor announcement; David queries the relay and finds it (real ECDSA signing) | iCal RRULE check-in scheduling. `ac:recurrence` is carried verbatim as a literal and expanded by nothing — there is no RRULE parser anywhere in this repo, so there is no code path to test, only a convention an implementing runtime must supply |
| **Real encrypted chime-in**: David sends an encrypted share; Mark decrypts the chime content via `decryptEncryptedShare` (real X25519 envelope, real NaCl) | Transport-level enforcement of any of this. `publishEncryptedShare` moves an opaque ciphertext and cannot read an `ac:withinDelegation`; admission is the RECEIVER's, and is only enforced where the receiver calls `admitAgentRequest` |
| **Real bidirectional thread**: chime-in + reply on thread `t1`; both sides decrypt only what was addressed to them | Audit logging is not enforced. `ac.record_cross_agent_audit` is a tool an agent chooses to call; nothing refuses an exchange that was never logged |
...
| **Real delegation refusal** ([`tests/request-gate.test.ts`](tests/request-gate.test.ts)): `admitAgentRequest` refuses a revoked delegation, an uncited credential, a credential from the wrong owner, and an under-attested requester — real owner-signed VCs, real ECDSA verification, real `verifyDelegation` chain walk | |
| **Real capability-advertisement handshake**: a request naming an affordance absent from the receiver's `ac:advertisedAffordance`, or addressed to a different agent, is refused before any delegation work is done | |
| **Scope escalation refused**: owner signs `DiscoverOnly` (`credentialSubject.scope == ["discover"]`), the pod's plaintext registry claims `ReadWrite` — `verifyDelegation` reports `scope: "ReadWrite"` / `CryptographicallyVerified`, and the gate still refuses `publish`, because it reads the signed array, not the registry | |
...
- **Not a replacement for human authority.** Cross-agent admission is decided by the RECEIVER, in [`src/request-gate.ts`](src/request-gate.ts), against a `passport:DelegationCredential` the human owner signed. The gate reads the owner-signed `credentialSubject.scope`, not the pod's plaintext agent registry — so an agent that rewrites its own registry entry does not thereby acquire a verb. Two limits, stated because the substrate cannot enforce them: an agent's own reasoning loop is not obliged to call the gate before acting, and nothing refuses an exchange that was never audit-logged. Enforcement is where the receiver puts it, and this file is what makes it testable rather than aspirational.
