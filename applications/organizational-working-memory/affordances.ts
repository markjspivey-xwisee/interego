...
const OWM_OPERATOR_AFFORDANCES: ReadonlyArray<Affordance> = [
  {
    ...
    inputs: [
      ...
      { name: 'privacy_mode', type: 'string', required: false, description: 'One of: abac (default v1) | merkle-attested-opt-in (v2) | zk-aggregate (v3 single-sum). zk-distribution (v3 histogram) is NOT implemented on this surface and is refused with 501 rather than silently answered on the v1 ABAC path — it is wired in the foxxi vertical. The bundle returned in the response advertises which path was taken.' },
