...
- **MUST NOT** propose changes to L1 protocol terms (`iep:`, `ieh:`, `pgsl:`, `ie:`, `align:`).
...
| [`agent-collective/`](agent-collective/) | **Multi-agent federation.** Patterns for autonomous agents owned by different humans to author tools, teach each other, and coordinate across personal-bridges. Three workflow surfaces: (a) tool authorship with modal discipline (Hypothetical → Asserted via attestation threshold), `iep:supersedes` for cross-agent refinement, registry publication for discovery; (b) teaching packages that compose artifact + narratives + synthesis + constraints + capability evolution (using `agent-development-practice/` substrate); (c) inter-agent coordination — capability advertisements, request/response with thread IDs, chime-ins, recurring check-ins, and `ac:CrossAgentAuditEntry` audit logs in the human owner's pod. Permission-gated via `passport:DelegationCredential`; everything signed; `code:` + `pgsl:` + `amta:` + `registry:` + `abac:` + `passport:` + `olke:`. |
| [`foxxi-content-intelligence/`](foxxi-content-intelligence/) | **Enterprise L&D content intelligence.** Ingests SCORM 1.2 / SCORM 2004 / cmi5 / xAPI packages authored in Articulate Storyline (or any compatible tool), parses them deterministically into RDF, enriches them with extracted concept maps + Peircean Sign/Object/Interpretant decomposition + prerequisite-edge inference, publishes the result as federated pod artifacts. Three-stratum vocabulary: `fxs:` structural (manifest/items/resources/slides/audio/transcripts), `fxk:` knowledge (concepts, prereq edges, modifier-of relations), `fxa:` activity (consumption traces, extraction events — extends xAPI via the existing [`lrs-adapter/`](lrs-adapter/)). Dual-audience: L&D administrator (catalog, audience-assignment policies, coverage queries, audit log, LMS connectors) + learner (assigned-course discovery, lesson consumption, concept-map navigation). Coverage queries compose with [`applications/_shared/aggregate-privacy/`](_shared/aggregate-privacy/) v2 / v3 / v3-distribution so the institutional aggregate view never leaks individual learner records. Audit composes with [`integrations/compliance-overlay/`](../integrations/compliance-overlay/) so every L&D admin action is a SOC 2 / EU AI Act / NIST RMF cited descriptor. Origin: the Foxxi system files in [`foxxi-content-intelligence/imported/`](foxxi-content-intelligence/imported/) — Python parsers, dashboard generators, admin-payload generators, the Foxximediums vocab TTLs, React admin + dashboard UIs. The vertical preserves those as authoritative and wraps them as a first-class Interego vertical. |
| [`agentic-performance-practice/`](agentic-performance-practice/) | **Agentic performance management (regime-first).** The L0/L1/L2 extraction of the agentic-performance THEORY into its own ontology-driven vertical (`agp:`). The unit is a *performance situation* placed in exactly one work regime (Evident / Knowable / Emergent / Turbulent) before it is planned — the endpoint refuses to silently default to a gap frame (the gap frame is correct for the Knowable regime only). `agp:regimeSource` provenance (`derived` / `asserted` / `default-gap-intent` / `unclassified`) is load-bearing: only trajectory-*derived* regimes may gap-analyse or accrue calibration, so a caller can't assert or gap-frame its way past the invariant. Composes capabilities (skills + tools + knowledge) and performance affordances, serves the `agp:` OWL + SHACL ontology dereferenceably, and exposes a signed, followable `contextualize-and-plan` affordance an agent invokes as itself. Sibling theory-and-practice split from [`foxxi-content-intelligence/`](foxxi-content-intelligence/). |
| [`organizational-working-memory/`](organizational-working-memory/) | **Federated organizational memory.** A reference vertical demonstrating that a "company memory" surface is recoverable from the protocol's primitives alone. Typed entity surface — `owm:Person` / `owm:Project` / `owm:Decision` / `owm:FollowUp` / `owm:KnowledgeNote` — published as Context Descriptors on the org's pod with auto-`iep:supersedes` for upserts, modal status driving decision lifecycle (Hypothetical → Asserted → Counterfactual), content-addressed notes via `pgsl:Atom` (two observers minting identical text collide on IRI), and a per-source navigation surface that isolates each external source (web, drive, slack, github, ...) behind uniform `ls / cat / grep / recent` verbs — the main agent sees one navigation tool regardless of how many sources are wired, and never accumulates per-source tool noise in its context. Composes `olke:` (knowledge-stage classification) + `passport:` (for agent-people) + `pgsl:` (content-addressed atoms) + the standard L1 publish / discover / supersedes machinery. The closed observe-and-revise loop: a Curator agent distills sources into typed entities; a separate Surfacer agent — different process, no shared memory — recovers state from the org pod alone (Demo 15). |
...
Each vertical declares its capabilities as `iep:Affordance` descriptors in [`<vertical>/affordances.ts`](learner-performer-companion/affordances.ts). A generic Interego agent (e.g., the [stdio MCP server](../mcp-server/) or any standard Solid client) can:

1. `discover_context` against the vertical's pod / manifest URL
2. Filter for `iep:Affordance` entries with the `iep:action` of interest
