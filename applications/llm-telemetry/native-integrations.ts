...
export function nativeIntegrations(base: string) {
  return { browser_extension: false, local_source_build: false, host_activation: 'not-verified',
    ...
    integrations: [
      ...
      { host: 'Ordinary Claude chat', method: 'Enterprise Compliance API', implementation: 'requires-enterprise-access-and-importer', live_host_test: false, requires: 'Enterprise organization, administrator-issued read:compliance_user_data key, own-user scope and a hosted importer. Endpoint contract is documented; no importer is active. Ordinary chat plugins do not run lifecycle hooks.', documentation: 'https://platform.claude.com/docs/en/manage-claude/compliance-content-data' },
    ],
