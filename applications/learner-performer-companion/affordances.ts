...
const LPC_ENTERPRISE_AFFORDANCES: ReadonlyArray<Affordance> = [
...

  {
    ...
    inputs: [
      ...
      { name: 'privacy_mode', type: 'string', required: false, description: 'One of: abac (default v1) | merkle-attested-opt-in (v2) | zk-aggregate (v3 single-sum) | zk-distribution (v3 histogram). The bundle returned in the response advertises which path was taken.' },
      { name: 'epsilon', type: 'number', required: false, description: 'DP ε budget for zk-aggregate / zk-distribution modes. Required when privacy_mode is one of those. For zk-distribution this is the per-bucket ε; histogram-level cumulative ε under sequential composition is k * ε where k = number of buckets.' },
      { name: 'distribution_edges', type: 'array', required: false, description: 'v3 zk-distribution: ascending bucket-edge boundaries (bigints) as decimal strings. Required when privacy_mode=zk-distribution. Each bucket is right-open except the last which is right-closed at distribution_max_value. For an edges array of length k+1 there are k buckets.' },
      { name: 'distribution_max_value', type: 'string', required: false, description: 'v3 zk-distribution: upper bound (decimal-string bigint) for the last bucket. Values above this throw at contribution time. Required when privacy_mode=zk-distribution.' },
