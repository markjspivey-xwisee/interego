...
machine — Windows 10 x64, electron-builder 25.1.8, Electron 33.4.11 — re-measured 2026-08-10.

| target | status here | evidence |
|---|---|---|
| **win** `zip` + `nsis` | **BUILT, INSTALLED FROM THE PUBLISHED RELEASE, LAUNCHED, AND PUBLISHED** | `Interego Workspace-0.2.0-win.zip` 128,004,481 B, `…-setup.exe` 92,182,214 B, both built from `1515bae` on a green master and attached to the `desktop-0.2.0` GitHub release. The evidence is not that they packed: the setup was **downloaded back from the release URL** (SHA256 `C787CB04…3D4E`, byte-identical to what was built), installed with `/S` over the running 0.1.0, launched, and it held a window titled `Interego Workspace`. It was then **closed** and exited in 0.5 s with no error dialog — the same gesture left 0.1.0 alive past 6 s, which is the defect #312 fixed. |
...
| **mac** `zip` + `dmg` | **BUILT; ONE OF TWO ARCHITECTURES LAUNCHED IN CI; UNSIGNED, UNPUBLISHED** | Built on a `macos-latest` runner by `desktop-package.yml`, which then starts the packaged `.app` directly (`…/Contents/MacOS/Interego Workspace`, via `tools/ci-launch-smoke.ts`) and asserts it reaches a window. ★ **AND THAT IS WHY NO macOS BINARY SHIPPED WITH `desktop-0.2.0`.** The leg produces **two** architectures — `release/mac/` (x64) and `release/mac-arm64/` — and the launch step is `app=$(ls -d "$rel"/mac*/*.app \| head -n1)`, which starts exactly ONE of them and never echoes which. So one of the two dmgs is a binary nobody has run, and the log does not say which one, so neither can be published. The fix is for that step to launch every `.app` it finds and print the path; until then this row stays UNPUBLISHED. Separately, the launch check never signs in, so `safeStorage` on macOS remains untested. |
