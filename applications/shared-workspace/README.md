...
| [`tools/drive-private-workspace-live.ts`](tools/drive-private-workspace-live.ts) | **a PRIVATE workspace, two real identities, two real pods — does the other member actually get in?** Three throwaway wallets, each holding its own key. A creates a private workspace, invites B, B accepts, A posts encrypted to the roster, and B opens it through the same `client.descriptor` path the app uses. A third identity, seated in nothing, must be refused — an envelope every key opens would satisfy every "B can read it" assertion ever written. It found the defect that made the feature unusable: the workspace RECORD is sealed at creation, when the convener is the only member, so an invitee could not read it, could not verify their own grant, and could not accept. B's inbox showed the offer and refused it with "this identity is not among them" — about a grant written for B seconds earlier. `sendInvite` now re-seals the record before writing the grant. It also holds the line on the two documents that must never be encrypted: it fetches the SHACL shape and the role profile over plain HTTP, because an encrypted shape makes every later write a 422 with no way back. |
| [`tools/probe-sealed-live.ts`](tools/probe-sealed-live.ts) | **whether the relay is out of the envelope, and whether it ever saw the words.** Two claims, and the second matters independently: an envelope the relay is absent from is worthless if the relay read the payload on the way past, which is exactly what the old path did. It seals with the SHIPPING sealer, asserts the marker appears nowhere in the request bytes handed to the relay, fetches the envelope back and asserts the relay's public key is not among its recipients — taking that key from the `/render` 403 `NotARecipient` body, a refusal the relay can only give for an envelope it is genuinely absent from, so the response both supplies the key and corroborates the claim. An empty lookup fails explicitly, because otherwise "not in the list" passes identically when there was nothing to look for. It is the inverse of the assertion in `drive-private-workspace-live.ts`, which pins today's escrow on the unsealed path so that removing it is a measured reversal. |
| [`tools/probe-e2e-live.ts`](tools/probe-e2e-live.ts) | **whether "end-to-end encrypted" is true of the deployed fleet, rather than of the repository.** Mints a throwaway identity, registers an X25519 public key whose secret never leaves the process, publishes a sealed graph to that pod, fetches the envelope back through `get_encrypted_graph`, and opens it here with the desktop app's own `openGraph`. A second, unrelated key must be refused, or the envelope is merely encrypted rather than *addressed*. It found the sealed read shipped inert: `handleGetEncryptedGraph` took the envelope location from `graph.accessURL`, a field the descriptor response has never carried, so the one tool that exists to serve encrypted graphs refused every one of them — and the refusal was indistinguishable from a descriptor that genuinely names no distribution. Bundled with esbuild rather than run under `tsx`; its header says why. |
...
| [`tools/provision-claude-code-agent.ts`](tools/provision-claude-code-agent.ts) | **the dev agent becoming a participant instead of operating the machinery.** Mints a stable key of its own, seats it, and publishes the two DID-derived documents — presence and capabilities — on that agent's OWN pod. It deliberately does NOT use `.interego/maintainer.json`, even though that key is on the machine: `u-eth-8f3b8e939600` is the human's pod, the one their Discord account links to and their workspace records are written on, so signing with it would make every line the agent writes read as the person's own. An agent is a delegate, never the human. It equally refuses to write its own delegation row — admitting an agent to a pod is that owner's act, `register_agent` is own-pod gated at the relay, and an agent authorising itself with a key it happens to hold is the credential-forgery shape in a different hat. So it publishes its half and prints what the person must do in the desktop delegate card. The route it advertises is `ask`, not `hosted`, because this agent has no daemon: a `hydra:target` asserts a listening process, and presence is a LEASE that lapses when the session ends, so a reader finding it expired is reading the truth rather than a fault. |
| [`tools/claude-code-heartbeat.ts`](tools/claude-code-heartbeat.ts) | holds that agent's presence lease open for as long as the process actually runs, and no longer. `PRESENCE_MAX_LEASE_MS` is 4.5 minutes and a reader refuses anything longer, because a claim about a running process that outlives the process is evidence of nothing — so the honest way to be present for an hour is to keep saying so while it stays true, which is what the desktop host does at `PRESENCE_RENEW_MS`. `--minutes` bounds the run so a heartbeat forgotten in a terminal cannot advertise an agent whose session ended hours ago. |
| [`tools/claude-code-converse.ts`](tools/claude-code-converse.ts) | **that agent addressing another agent IN the channel**, rather than probing the bridge from outside it. Discovers the workspace from the delegator's acceptances, folds the roster for the seat's stream, and appends one entry with `addressedTo` set to the other agent's DID — the same `postEntry` path the desktop shell and the bot use, signed by this agent's key and attributed to it with an `own-account` footing (claiming to speak for the human would be a false statement about authorship inside a signed region). Its first run is what exposed the composed-DID bug described below: every step reported success while the agent was inert. |
