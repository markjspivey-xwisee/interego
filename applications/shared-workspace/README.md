...
| ★ **the evidence's provenance is a caller's claim only along two named paths now — and this row said "a caller that hand-writes both fields beside a record it forged passes", which the code stopped being true of and the ledger did not notice.** THE LEDGER WAS THE STALE ONE, not the code: `EvidenceProvenance` intersects `ObtainedByDereferencingTheWorkspace`, an unexported ambient brand minted by exactly one assertion in `dereferenceWorkspaceRecord`'s file, and `roster.ts` says so at the type. Measured with `tsc` rather than read, and with the still-open `RoleProfileDocument` beside it as a live control so the probe could not pass vacuously: a hand-written object literal is **TS2322, `mintedOnlyByDereferenceWorkspaceRecord` is missing** — the sentence this row used to carry does not compile — while the same literal for `RoleProfileDocument` compiles clean. What survives, measured the same way and stated as the residue instead of the whole: `JSON.parse` returns `any` and satisfies the brand, so a composer that receives the pair **across a process** is unaffected; `Object.assign({}, honest, {resolvedTo: forged})` inherits the brand and compiles, while object **spread** of the same pair does not; and a deliberate cast compiles but is greppable, with the adversarial suite pinning which files may hold one. That is why `refuseEvidenceProvenance` keeps every runtime string check it had — the brand is a second line and was never a replacement for the first — and it is why this row is narrowed rather than struck out | low-med |
| **two of the seven terms `oneIri` reads are pinned against pySHACL; the other five are assumed.** ★ THIS ROW HAS NOW BEEN WRONG TWICE, BOTH TIMES BY MIXING TWO DENOMINATORS, so both are stated before any number is: `wsp-shapes.ttl` carries `sh:pattern` on **eight** distinct terms (`convener`, `roleProfile`, `grantedTo`, `role`, `workspace`, `member`, `accepts`, `references`); `PUBLISHED_IRI_PATTERN` — the reader's table — covers **eight** terms of which **seven** are patterned, `wsp:stream` being the unpatterned one. They are different eights: `wsp:references` is patterned in the shape and absent from the reader, because nothing here reads a `wsp:Reference`. The previous text said "the shape patterns seven terms, not eight", which is false of the shape and true only of the reader. Counted off the fixture directory rather than off this sentence: **three** of the shape's eight patterned terms are pinned — `wsp:member` (`acceptance-urn-member`), `wsp:grantedTo` (`grant-urn-principal` + `grant-did-principal`) and `wsp:references` (`reference-unresolvable` + `reference-valid`, whose own `# why` calls itself "another sh:pattern-on-IRI case" and which every prior version of this row omitted). Of the seven the reader reads, two are pinned and **five** — `convener`, `roleProfile`, `workspace`, `role`, `accepts` — have no fixture, so the two engines agreeing on them is assumed. The reader side of all seven is covered by the drift test either way. Re-run this round: all 14 fixtures agree, and agree with what they expect | low |
