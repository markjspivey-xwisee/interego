...
AGP owns the performance consulting and management model and the performance HTTP implementation in `bridge/performance-compat-routes.ts`. FOXXI keeps a compatibility export and its existing URLs for content, assessments and standards delivery. Both authorities declare their own action names with the same input and output contracts. The same methods apply to humans, agents and mixed teams; they do not add domain-specific tools to the neutral Interego core.
