...
import type { IRI, IPFSAnchor } from '@interego/core';
 ...
 *
 * Lives in `@interego/pgsl` as `computeLatticeCids` — that overload
 * accepts the PGSL instance directly. This duck-typed wrapper accepts
 * anything with a `nodes` Map<IRI, {kind, ...}> shape so the Solid
 * client can compute CIDs over an externally-provided lattice without
 * the substrate having to depend on `@interego/pgsl` at the type level.
 */
export interface LatticeLikeForCids {
  readonly nodes: Map<IRI, {
