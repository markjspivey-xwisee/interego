...
import { validate } from '@interego/core';
import { publish, discover, subscribe as solidSubscribe } from './client.js';
import { computeCid, pinToIpfs } from '@interego/core';
...
.map(s => ({
  ...
  // PGSL convenience methods (`ingest`, `meet`, `resolve`, `stats`)
  // previously lived here. They moved out with the rest of PGSL — see
  // `@interego/pgsl` (`embedInPGSL`, `latticeMeet`, `resolve`,
  // `latticeStats`). Use those directly:
  //
  //     import { createPGSL, embedInPGSL } from '@interego/pgsl';
  //     const pgsl = createPGSL({ wasAttributedTo: 'urn:agent:me' });
  //     const uri = embedInPGSL(pgsl, 'hello world');
