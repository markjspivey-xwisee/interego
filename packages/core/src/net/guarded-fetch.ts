/**
 * SSRF guard for caller-supplied URLs any surface fetches server-side.
 *
 * ── ★★ MOVED HERE BECAUSE IT IS NOT ONE VERTICAL'S PROBLEM ──────────────────────────────────
 *
 * This lived in the Foxxi bridge. A system audit found the same screen independently implemented at
 * the relay and — the part that matters — ABSENT from a third vertical that fetches caller-supplied
 * source URLs. Every surface that resolves a URL from caller input and then fetches it needs this;
 * a screen that ships with one application is a screen the next application does not get.
 *
 * ★ THE RELAY'S EGRESS MODULE IS DELIBERATELY NOT FOLDED IN. It is a superset — pinned CSS origin,
 * relay-loopback allowance, redirect budget — and its own header records TWO production outages
 * caused by changing it, both from a container-only undici failure that reproduced nowhere else.
 * Consolidating a correct 476-line module with that history into this one, to remove a duplication
 * that costs nothing at runtime, is a bad trade. The screen below is the portable half; the relay
 * keeps the deployment-shaped one.
 *
 * ── original header ──
 *
