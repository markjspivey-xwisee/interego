 ...
 *   - Built-in (domain-neutral) decorators cover: core HATEOAS, ontology
 *     patterns, coherence, persistence, LLM advisory, and federation. Vertical
 *     domain decorators (e.g. Foxxi's xAPI expert) are registered by the vertical
 *     on top of these, never built into the substrate.
 *
...
// Exported so vertical decorators (e.g. Foxxi's xAPI domain expert) can be
// authored OUTSIDE the substrate and compose the framework — keeping
// domain vocabularies (xAPI/cmi5/SCORM) in their vertical, not in PGSL.
...
// ── 3. (xAPI Domain Expert Decorator — MOVED OUT OF THE SUBSTRATE) ──
//
// xAPI is a Foxxi-VERTICAL concern, not an Interego/PGSL foundation primitive.
// The xAPI domain decorator (verb vocabulary, result/context properties, ADL
// verb URIs) now lives in the Foxxi vertical and is registered onto the
// decorator chain from there — composing the framework above (AffordanceDecorator
// + makeAffordance) without baking a learning-domain vocabulary into the
// substrate. See applications/foxxi-content-intelligence/src/xapi-decorator.ts.
...
export const llmAdvisorDecorator: AffordanceDecorator = {
  ...
  decorate(context: DecoratorContext): DecoratorResult {
    ...
    lines.push(`5. Are there ontology or domain-specific patterns that could be applied?`);
...
/**
 * Create a registry pre-loaded with the substrate's domain-neutral built-in
 * decorators (core HATEOAS, ontology, coherence, persistence, federation, LLM
 * advisory). Vertical decorators — e.g. Foxxi's xAPI domain expert — are
 * registered by the vertical on top of this via registerDecorator(), so no
 * learning-domain vocabulary ships in the foundation.
 */
