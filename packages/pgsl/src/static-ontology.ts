...
/**
 * Resolve the docs/ns/ directory.
 *
 * Walks upward from this module's location looking for a `docs/ns`
 * folder. This finds the workspace-root copy regardless of whether
 * the substrate is run from source (packages/core/src/pgsl/) or
 * compiled (packages/core/dist/pgsl/) and regardless of where on
 * the filesystem the workspace is checked out. Falls back to the
 * legacy "two-levels-up" lookup for downstream consumers that
 * vendor the package with its own docs/ns/ tree.
 */
function resolveNsDir(): string {
  const here = dirname(fileURLToPath(import.meta.url));
  // Walk up at most 8 levels — covers source/dist/* layouts.
  let dir = here;
  for (let i = 0; i < 8; i++) {
    const candidate = resolve(dir, 'docs', 'ns');
    if (existsSync(candidate)) return candidate;
    const parent = resolve(dir, '..');
    if (parent === dir) break;
    dir = parent;
  }
  // Legacy fallback (vendored package).
