...
npx -y @foxxi/context-graphs-mcp
```

Or pin it globally:

```bash
npm install -g @foxxi/context-graphs-mcp
context-graphs-mcp
```
...
npm install
npm run build
npm pack   # creates foxxi-context-graphs-0.2.0.tgz

# Then build the MCP server against the local library
cd mcp-server
npm install ../foxxi-context-graphs-*.tgz --no-save
