...
const BODIES = {
  ...
  claim() {
    ...
    if (d.credential) {
      const vc = d.credential.vc ?? {};
      const subject = vc.credentialSubject ?? {};
      const ach = subject.achievement ?? {};
      out.push(h('div', { class: 'badge' },
        ...
        h('div', {},
          ...
          h('dl', { class: 'kv' },
            ...
            h('dt', {}, 'in your wallet'), h('dd', {}, link(d.credential.descriptorUrl, d.credential.descriptorUrl)),
            h('dt', {}, 'proof'), h('dd', {}, vc.proof?.cryptosuite ?? vc.proof?.type ?? '')))));
      out.push(raw('The signed credential', vc));
    }
...
function renderChapters() {
  $('chapters').replaceChildren(...CHAPTERS.map((c) => {
    const status = chapterStatus(c.id);
    return h('section', { class: `chapter ${status}`, id: `ch-${c.id}` },
      ...
      status === 'locked' ? null : h('div', { class: 'chapter-body' }, BODIES[c.id]()));
  }));
}
...
function render() {
  ...
  renderCast();
  renderFacts();
  // Keep focus and caret in a text field across re-renders.
  const active = document.activeElement;
  const focusKey = active?.tagName === 'INPUT' ? [...document.querySelectorAll('input')].indexOf(active) : -1;
  const caret = focusKey >= 0 ? active.selectionStart : null;
  renderChapters();
  if (focusKey >= 0) {
    const again = document.querySelectorAll('input')[focusKey];
