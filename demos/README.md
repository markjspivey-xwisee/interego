...
| 02 | [Path B — named MCP tools](scenarios/02-path-b-named-tools.ts) | Same end result via the per-vertical bridge's named MCP-tool surface. The bridge derives MCP tool schemas from the same `iep:Affordance` declarations Path A walks — single source of truth, two surfaces. |
...
| # | Scenario | What it demonstrates |
|---|----------|----------------------|
| 05 | [Time-paradox memory](scenarios/05-time-paradox-memory.ts) | A Hypothetical claim collapses to Counterfactual via `iep:supersedes`. The agent's *current* view of a claim is the head of the supersedes chain; older descriptors remain on the pod (audit trail) but no longer represent live belief. The substrate makes belief revision legible without coordination. |
