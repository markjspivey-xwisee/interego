...
az containerapp create \
  ...
  --cpu 0.25 \
  --memory 0.5Gi \
  --env-vars "CSS_URL=$CSS_INTERNAL_URL" "PORT=5000" "POD_NAME=$POD_NAME" \
  --output none
...
FB_PLAIN_VARS=(FOXXI_TENANT_POD_URL FOXXI_AUTHORITATIVE_SOURCE FOXXI_AUDIENCE FOXXI_DASHBOARD_ORIGIN BRIDGE_DEPLOYMENT_URL FOXXI_ADMIN_WEB_ID FOXXI_REQUIRE_AUTH FOXXI_TENANT_PROFILE_NAME FOXXI_LEARNING_ENGINEER_WEB_IDS FOXXI_FEDERATION_PODS FOXXI_AGENTIC_RATE_LIMIT_PER_IP FOXXI_LRS_BACKEND FOXXI_DASHBOARD_URL FOXXI_TENANT_PROFILE_DID FOXXI_LRS_FORWARDING_TARGETS FOXXI_LRS_OAUTH_CLIENTS FOXXI_LTI_PLATFORMS FOXXI_SCORM_PLAYER_BASE)
FB_SECRET_VARS=(FOXXI_LLM_API_KEY FOXXI_ADMIN_KEY_SEED FOXXI_ISSUER_KEY_SEED FOXXI_BRIDGE_PRIVATE_KEY FOXXI_POD_WRITE_SECRET FOXXI_LRS_BASIC_AUTH_PAIRS FOXXI_WALLET_SEED FOXXI_LTI_KEY_SEED FOXXI_LTI_PRIVATE_KEY_PEM FOXXI_SCORM_CLOUD_APP_ID FOXXI_SCORM_CLOUD_SECRET_KEY)
FB_SET_ARGS=()
