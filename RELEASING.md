...
- **`@foxxi/context-graphs`** — the TypeScript library (root `package.json`)
- **`@foxxi/context-graphs-mcp`** — the MCP server for AI agents (`mcp-server/package.json`)
...
1. Sign in at [npmjs.com](https://www.npmjs.com/) (must be a maintainer of the `@foxxi` org)
...
3. Settings:
   ...
   - **Packages and scopes:** select `@foxxi/context-graphs` and `@foxxi/context-graphs-mcp`
...
### `@foxxi/context-graphs` tarball contents
...
### `@foxxi/context-graphs-mcp` tarball contents
...
### "@foxxi/context-graphs@X.Y.Z is not in this registry"

The `publish-mcp-server` job waits up to 5 minutes for the library to propagate. If it times out, the npm registry was unusually slow or the library publish failed. Re-run the workflow with `gh run rerun <run-id>` once the library shows up at `npm view @foxxi/context-graphs`.

### "403 Forbidden" on publish

Either the `NPM_TOKEN` secret is missing/expired, or it doesn't have write access to the `@foxxi` scope. Regenerate the token (see one-time setup above) and `gh secret set NPM_TOKEN` again.
