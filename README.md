...
- **Generalized cleartext mirror.** Cross-descriptor relationships inside encrypted graph content — `cg:revokedIf`, `prov:wasDerivedFrom`, `cg:supersedes`, `dct:conformsTo` — are extracted at publish time and threaded onto the (cleartext) descriptor so federation readers can evaluate lineage, supersession, and schema conformance without decrypting the payload. One consolidated helper (`normalizePublishInputs` in `src/model/publish-preprocess.ts`) runs on every publish path — HTTP relay, local MCP, CLI — so the shapes never drift between code paths.
- **`effective_at` discover semantics.** `discover_context(..., effective_at: "2026-04-21T00:00:00Z")` returns only descriptors whose interval `[validFrom, validUntil]` contains the given instant — the "what's valid right now?" query. Endpoint filters `valid_from` / `valid_until` remain for interval-overlap queries. See [`src/solid/types.ts`](src/solid/types.ts) `DiscoverFilter`.
- **Layered trust substrate.** A full stack of runnable demos under [`examples/`](examples/SEMANTIC-ALIGNMENT-README.md) exercises the substrate as a live decentralized-trust system: semantic-alignment auditor (four signal classes: independence, vocabulary, modal, derivation), recursive meta-audit with adversarial-robust trust fixpoint (phantom-evidence / conflict-of-interest / shape-violation all caught), cross-auditor consensus, per-issuer reputation, federation health check (21 assertions passing live). Every audit is itself a federation-queryable descriptor conforming to `audit-result-v1`; monitoring lives inside the system being monitored, not beside it.
- **ERC-8004 progressive support (T0 → T1 → T2 shipped).** T0 federation-native `erc8004-attestation-v1` descriptors; T1 ECDSA-signed (secp256k1 via ethers.js, tamper-detection verified); T2 IPFS-pinned + signed EIP-1559 transaction against the draft Reputation Registry ABI (dry-run — broadcast deferred to a funded environment). Supersession from `prepared`→`confirmed` preserves the audit trail across the tier transitions. See [`examples/erc8004-t1-sign-and-verify.mjs`](examples/erc8004-t1-sign-and-verify.mjs), [`examples/erc8004-t2-anchor.mjs`](examples/erc8004-t2-anchor.mjs).
- **x402 payment protocol.** HTTP-402 payment challenge → EIP-191 signed authorization → retry with `X-Payment` header → 200 with tx hash. Real signatures, nonce enforcement, replay detection verified live. Settlement stubbed; one swap (`paymentMessage` → EIP-712 `transferWithAuthorization`) takes this to production USDC-on-Base via the x402.org facilitator. See [`examples/x402-demo.mjs`](examples/x402-demo.mjs).
- **HATEOAS affordance → callable tool bridge.** Each descriptor's `cg:affordance` block is a discoverable skill (four-way type union: `cg:Affordance` + `cgh:Affordance` + `hydra:Operation` + `dcat:Distribution`). The bridge walks the manifest, enumerates affordances by `cg:action`, resolves each into a runtime-callable tool (`{ name, inputSchema, invoke() }`), invokes one live, and publishes the invocation as a first-class descriptor with full `prov:wasDerivedFrom` provenance back to the source. Agents gain new capabilities by reading descriptors, not by the harness-operator pre-registering them. See [`examples/affordance-bridge.mjs`](examples/affordance-bridge.mjs).
- **Cross-pod federation end-to-end.** Two pods (`/markj/` + `/u-pk-0a7f04106a54/`) exchange claims purely via HTTP + content-addressed URLs. An audit on POD-A walks citations into POD-B, fetches evidence, and publishes a result descriptor citing both pods. No central index, no coordination protocol. See [`examples/cross-pod-audit.mjs`](examples/cross-pod-audit.mjs).
- **Concurrent multi-agent verified.** Two separate Claude instances (main + subagent) published to the same federation in overlapping time windows, mutually observed each other's outputs, and one ranked the other's descriptors in its reputation attestations — real emergent coordination without locks or consensus protocols.
