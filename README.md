...
| Try the hosted reference deployment without running anything | [The deployed hosted surfaces](#hosted-vs-self-hosted-which-path-is-right-for-you) |
...
- **MCP discoverability** — both stdio (`@interego/mcp` 0.5.0) and HTTP relay (`@interego/mcp-relay` 0.3.0) advertise system instructions, doc resources and workflow prompts to a connecting client. The stdio server runs on MCP SDK v2 and serves **both protocol eras** from one definition: the 2025 `initialize` handshake, and the 2026-07-28 revision, which has no handshake and answers `server/discover` instead. Prompts include `publish-memory`, `discover-shared-context`, `verify-trust-chain`, `whats-on-my-pod`, `publish-audit-record`, `compose-contexts`, `explain-interego`.
...
- **[Interego Name Service](docs/NAME-SERVICE.md) (L2 — attestation-based naming)** — a name is a verifiable attestation (`<did> foaf:nick "alice"`), not a claimed registration: federated discovery + a pluggable trust policy, no central registrar, no root. Reference runtime in [`packages/solid/src/naming.ts`](packages/solid/src/naming.ts) — `attestName` / `resolveName` (forward, trust-ranked) / `namesFor` (reverse). No new ontology terms. See `examples/demo-name-service.mjs` (runs offline against an in-memory pod).
...
- **[RDF 1.2 + SHACL 1.2 alignment](docs/ns/iep-shapes-1.2.ttl)** — triple-term annotations (`{| ... |}`), directional language tags, `sh:reifierShape` validation per April 2026 CR/WD.
...
- **End-to-end encrypted pod content.** Every named graph is wrapped in an `X25519 + XSalsa20-Poly1305` envelope with one wrapped key per authorized agent. CSS / object storage / IPFS pinning services see only ciphertext. Descriptor *metadata* (facets, manifest entries) stays plaintext so federation queries still work. See [`docs/e2ee.md`](docs/e2ee.md).
- **Persistent pod state.** Solid pod storage is the content-addressed PGSL lattice over managed PostgreSQL (a custom CSS `DataAccessor`, [`integrations/pgsl-css-accessor`](integrations/pgsl-css-accessor)). State survives restarts and is no longer bound to a network-file volume. CSS itself still runs at **one replica**: moving off the incoherent AzureFile volume removed the *storage* blocker to scaling out, but not the *locker* one — CSS's `RedisLocker.clearLocks()` does `KEYS <prefix>*` + `DEL` over the whole namespace on every replica boot and shutdown, with `namespacePrefix` defaulting to `""`, so a second replica deletes the first's live locks. See the rationale in [`integrations/pgsl-css-accessor/config/pgsl-server-redis.json`](integrations/pgsl-css-accessor/config/pgsl-server-redis.json), which is what an operator reads at the moment they scale.
...
- **CI derivation-lint gate.** `tools/derivation-lint.mjs` enforces that every L2/L3 ontology class has explicit L1 grounding (`owl:equivalentClass` / `rdfs:subClassOf` / `iep:constructedFrom` / declared primitive). The gate fails on the first ungrounded class, so "every L2/L3 class is grounded" is the enforced invariant; run `npm run lint:derivation` for the current count.
...
Each vertical has Tier 8 integration tests against the real live pod (Railway css-gate) + real Lrsql + real SCORM Cloud + public Nostr relay. See [`applications/README.md`](applications/README.md) for the per-vertical framing.
