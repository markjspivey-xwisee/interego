...
import { parseSignedJsonDocument, type Json } from '../../integrations/application-runtime/application-lab-runtime.js';
...
export async function signingFixture() {
  ...
  const context = (credential = 'alice'): ResourceWriteContext => ({ reads: { ...store.reads, discover: store.reads.discoverCatalogs },
    principal: owners[credential]!.principal, identityUrl: 'https://identity.example', now: new Date(now).toISOString(),
  ...
  return { broker, deps, storage, store, registry, initial, context, create, control, sign, owners, wallets, publish, revoked,
