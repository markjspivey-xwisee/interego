...
describe('binary artifact bytes survive both affordance invocation paths', () => {
  it('preserves bytes through the production relay adapters, including a redirect, over real HTTP', async () => {
    const server = createServer((req, res) => {
      if (req.url === '/redirect') { res.writeHead(302, { Location: '/file' }); res.end(); return; }
      if (req.url === '/descriptor') { res.setHeader('Content-Type', 'text/turtle'); res.end(descriptor.replace('https://example.test/file', base + '/redirect')); return; }
      res.setHeader('Content-Type', 'application/zip'); res.end(Buffer.from(bytes));
    });
    await new Promise<void>(resolve => server.listen(0, '127.0.0.1', resolve));
    const base = `http://127.0.0.1:${(server.address() as { port: number }).port}`;
    const egress = createEgress({ cssUrl: base, publicBaseUrl: base, screenAddresses: true });
    try {
      for (const fetcher of [egress.solidFetch, egress.guardedInvokeFetch]) {
        const direct = await act({ target: (base + '/redirect') as IRI, action: 'urn:download' as IRI, method: 'GET' }, {}, { fetch: fetcher });
        expect(direct.bodyEncoding).toBe('base64'); expect(Buffer.from(direct.body, 'base64')).toEqual(Buffer.from(bytes));
        const followed = await followAffordance(base + '/descriptor', 'urn:download', {}, { fetch: fetcher });
        expect(followed.bodyEncoding).toBe('base64'); expect(Buffer.from(followed.body, 'base64')).toEqual(Buffer.from(bytes));
      }
    } finally { await egress.close(); await new Promise<void>(resolve => server.close(() => resolve())); }
  });
