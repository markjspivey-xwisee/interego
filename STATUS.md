...
| Future v3.3 — signed audit-log descriptor on the institution's pod | v3.2 ships `EpsilonBudget.toJSON()` so the consumption log can be serialized; v3.3 wraps that in a signed pod descriptor so an auditor can verify the log itself wasn't tampered with after the fact. Composes existing `src/compliance/signDescriptor` + the standard `publish()` path. |
