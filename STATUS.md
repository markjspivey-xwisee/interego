...
| v4 (in progress) — Shamir SS primitive shipped, DKG + protocol-wiring still future | [`src/crypto/shamir.ts`](src/crypto/shamir.ts) ships the splitting + Lagrange-reconstruction over the ristretto255 scalar field (18 contract tests in [`tests/shamir.test.ts`](tests/shamir.test.ts) — round-trip, threshold strictness, validation, corrupt-share behaviour, hiding smoke check). This is the algebraic building block for v4 multi-aggregator threshold reveal; the full v4 still needs (a) Distributed Key Generation to remove the trusted dealer and (b) protocol wiring into `buildAttestedHomomorphicSum` so the trueBlinding is reconstructed from a t-of-k committee instead of held by a single aggregator. |
