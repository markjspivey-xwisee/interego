...
**Scope:** Declarative self-revocation of context graphs. A claim publishes, alongside itself, the conditions under which it ceases to be true. Federation-layer readers evaluate the condition without needing to decrypt the payload. When the condition is satisfied, the claim's effective `iep:groundTruth` transitions according to the declared action.

**Out of scope:** Unilateral revocation by the owner (already covered by `revoke_agent` + `validUntil`), cryptographic revocation tokens (out of scope for this spec; potentially a future extension), and revocation of a claim by its *successor* claim (that's `iep:supersedes`, already in v1.0).
...
|---|---|---|
| `iep:revokedIf` / `iep:revokedBy` | `iep:SemioticFacet.revokedIf` (Proposal B) or `iep:RevocationFacet` (A) | yes |
| `prov:wasDerivedFrom` | `iep:ProvenanceFacet.wasDerivedFrom` | yes |
| `iep:supersedes` | descriptor-level `iep:supersedes` | yes (manifest) |
...
Both proposals use the same `iep:RevocationCondition` structure. This is deliberate — a condition written under one proposal is a blank-node rewrite away from the other. Implementations MUST treat these terms as portable across whichever of A or B is adopted.
...
[] a iep:RevocationCondition ;
    ...
    """ ;
    iep:evaluationScope iep:KnownFederation ;
    iep:onRevocation iep:MarkInvalid ;
    iep:revocationIssuer <https://id.example.com/users/alice/profile#me> .
```

### 2.1 `iep:successorQuery`

- MUST be SPARQL 1.1 ASK / SELECT / CONSTRUCT.
- MUST NOT reference the enclosing descriptor's own graph IRI (self-revocation-by-existence is malformed; caught by `iep:RevocationConditionNoSelfReferenceShape` in [`cg-shapes.ttl`](../docs/ns/cg-shapes.ttl)).
...
### 2.2 `iep:evaluationScope`

Closed vocabulary:

| Value | Evaluation surface | Expected latency |
|---|---|---|
| `iep:LocalPod` | Descriptors on the declaring pod only | Cheap — no federation calls |
| `iep:KnownFederation` | Every pod in the reader's known-pods registry | Moderate — cached federation state acceptable; eventual consistency OK |
| `iep:WebFingerResolvable` | Any pod reachable via WebFinger / DID resolution from handles cited in the query | Expensive — used only when the strongest guarantee is needed |

### 2.3 `iep:onRevocation`

Closed vocabulary. Implementations MUST support `iep:MarkInvalid`; the others are RECOMMENDED:

| Action | Effect |
|---|---|
| `iep:MarkInvalid` | Effective `iep:groundTruth` → `false`. Modal status behaves as Counterfactual for currently-valid queries. |
| `iep:DowngradeToHypothetical` | Modal status → `iep:Hypothetical`; `groundTruth` → undefined. Signals unsettled truth. |
| `iep:RequireReconfirmation` | Modal status preserved; descriptor flagged as requiring issuer re-signature before use for downstream claims. |

### 2.4 `iep:revocationIssuer`
...
## 3. Proposal A — seventh facet `iep:RevocationFacet`
...
<urn:iep:example:1> a iep:ContextDescriptor ;
    iep:hasFacet [ a iep:TemporalFacet ; ... ] ;
    iep:hasFacet [ a iep:ProvenanceFacet ; ... ] ;
    iep:hasFacet [ a iep:AgentFacet ; ... ] ;
    iep:hasFacet [ a iep:SemioticFacet ; ... ] ;
    iep:hasFacet [ a iep:TrustFacet ; ... ] ;
    iep:hasFacet [ a iep:FederationFacet ; ... ] ;
    iep:hasFacet [
        a iep:RevocationFacet ;
        iep:revokedBy [
            a iep:RevocationCondition ;
            iep:successorQuery "..." ;
            iep:evaluationScope iep:KnownFederation ;
        ...
        iep:revocationIssuer <https://regulator.example.com/profile#me>
    ] .
```
...
- **Independent issuer.** The facet's `iep:revocationIssuer` MAY point to a third party — a regulator or auditor can attach a revocation policy to a claim whose author they have no control over.
...
## 4. Proposal B — predicate `iep:revokedIf` on `iep:SemioticFacet`

### 4.1 Structure

`iep:revokedIf` is an object property whose domain is `iep:SemioticFacet` and range is `iep:RevocationCondition`. The condition is nested inside the facet that already owns the descriptor's truth state.
...
- **Not cheaply facet-type-filterable.** `discover_context(facet_type=Revocation)` is not a thing; instead readers filter `facet_type=Semiotic` and inspect the `iep:revokedIf` sub-predicate. Every descriptor has a SemioticFacet, so the filter is not selective.
- **Implicit issuer.** Without a separate facet, there's no natural attachment point for `iep:revocationIssuer` — the enclosing descriptor's author is assumed. Third-party revocation policies don't fit.
...
1. Before deciding whether a descriptor's claim is currently valid, evaluate every attached `iep:RevocationCondition` against its declared `iep:evaluationScope`.
2. If any condition's `iep:successorQuery` returns a non-empty result (ASK → true, SELECT → ≥ 1 binding, CONSTRUCT → ≥ 1 triple), apply the associated `iep:onRevocation` action.
...
<...> a iep:ContextDescriptor ;
    iep:hasFacet [
        a iep:SemioticFacet ;
        iep:revokedIf [ ... Condition-B ... ]
    ] ;
    iep:hasFacet [
        a iep:RevocationFacet ;
        iep:revokedBy [ ... Condition-A ... ]
    ] .
```
